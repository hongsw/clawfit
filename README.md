...
## 🔥 What's hot right now (2026-07-21)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[JustVugg/colibri ⭐17,430](https://github.com/JustVugg/colibri)** | Pure-C, zero-dep inference engine for running GLM-5.2 (744B MoE) on 25GB RAM by streaming expert weights from disk. Three-tier memory hierarchy (VRAM→RAM→storage); learning cache; 71.6% prefetch predictability; 0.05–0.1 tok/s baseline. First MoE-native consumer runtime in the scan corpus. **Two-signal "MoE-native consumer inference" condition met** with ktransformers (2026-07-19). v1.0.0, 19 days old. | L1 |
| **[XiaomiMiMo/MiMo-Code ⭐12,297](https://github.com/XiaomiMiMo/MiMo-Code)** | Xiaomi's terminal coding agent (OpenCode fork) with three-role permission model (`build`/`plan`/`compose`), persistent cross-session memory (MEMORY.md + SQLite FTS5), deterministic JS workflows, 30+ named skills, and voice input (TenVAD + MiMo ASR). Adds structured memory and role-based access to OpenCode's base without cloud dependency. | L2/L5 |
| **[omnigent-ai/omnigent ⭐7,576](https://github.com/omnigent-ai/omnigent)** | Meta-harness orchestrating Claude Code, Codex, Cursor, OpenCode, Hermes, and Pi under one YAML-defined session model. Supervisor pattern native; cross-device session sync (terminal→browser→mobile); cloud sandbox delegation (Modal, Daytona, E2B, Kubernetes); governance layer (shell-command approval, per-agent spend caps). **Two-signal "meta-harness for multi-agent coding orchestration" confirmed** with loop-engineering (2026-07-05). v0.6.0, alpha. | L2/L3 |
| **[unicity-aos/aos-ce ⭐5,405](https://github.com/unicity-aos/aos-ce)** | Open agent OS in Rust: capsule model (least-privilege user-space building blocks), Forge self-extension tooling (agents inspect the OS and build verified capsules at runtime), Sigstore-signed releases, runtime compatibility gates. Claims to sit below the L2 framework layer as an OS-level substrate. 9 days old at time of scan. | L1/L2 |
| **[shepherd-agents/shepherd ⭐1,513](https://github.com/shepherd-agents/shepherd)** | Runtime substrate making agent execution reversible: proposals staged before file writes, copy-on-write forking (~5× Docker speed), 95% KV-cache reuse on replay. OS-level permission grants (macOS Seatbelt, Linux Landlock). Designed to support MCTS-RL meta-agent supervision — tree-search over agent paths as a data structure operation. First reversible execution substrate in the scan corpus. | L5/L2 |
| **[langchain-ai/openwiki ⭐12,600](https://github.com/langchain-ai/openwiki)** | CLI that auto-generates agent-consumable codebase wikis (Open Knowledge Format). CI/CD native. 12.6k stars in 15 days. **Two-signal "agent-optimized documentation layer" pattern confirmed** with Agent Docs for Markdown (2026-07-19). | L5 |
| **[langchain-ai/open-swe ⭐10,400](https://github.com/langchain-ai/open-swe)** | Background-async SWE agent reference implementation (Stripe/Ramp/Coinbase pattern). Tasks to cloud sandbox (Modal/Daytona/E2B), unsupervised, surface as draft PR. **Two-signal "background-async SWE harness" confirmed** with background-agents (2026-07-13). | L2 |
| **[PrefectHQ/fastmcp ⭐26,500](https://github.com/PrefectHQ/fastmcp)** | Dominant Python MCP framework (~70% of all MCP servers, ~1M downloads/day). v3.4.x: SSRF/OAuth hardening, `fastmcp-remote` bridge, Prefect Horizon enterprise gateway. Defines the quality floor for L4 MCP entries. | L4 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (498 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
