...
## 🔥 What's hot right now (2026-07-19)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[handy-computer/transcribe.cpp ⭐811](https://github.com/handy-computer/transcribe.cpp)** | Multi-model local ASR library: 16 model families, 60+ variants (Whisper, Moonshine, Parakeet, Canary…), Metal/Vulkan/CUDA/TinyBLAS backends, Python/TypeScript/Rust/Swift bindings. WER-tested across model×hardware matrix. v0.1.3 July 12. HN 670pts — highest story today. Drop-in whisper.cpp replacement with far broader model coverage. Two-signal local ASR pattern confirmed with moonshine-micro (same day). | L7 |
| **Qwen3.8 (Alibaba)** | 2.4T-parameter flagship preview announced July 19, claimed "second only to Fable 5." No published benchmarks, no open weights yet ("soon"), no HuggingFace model card. MoE configuration not disclosed — "2.4T" is a headline number. Currently bundled in Token Plan subscriptions ($6–$68/week). Same-day competitive response to Kimi K3 demand spike. HN 560pts. Hold for open-weight confirmation. | L1 hold |
| **[kvcache-ai/ktransformers ⭐18.3k](https://github.com/kvcache-ai/ktransformers)** | CPU-GPU heterogeneous MoE inference: AMX/AVX-optimized CPU kernels for cold experts + CUDA for hot experts. 4.62–19.74× prefilling speedup (ACM SIGOPS 2026 peer-reviewed). Targets DeepSeek-V3/R1 and MiniMax-M3 on consumer hardware. GOSIM Paris 2026 "Agentic AI on Edge" track. Two-signal consumer local large-model inference pattern with airllm (same day). Apache 2.0. | L7 |
| **[agentscope-ai/QwenPaw ⭐23.5k](https://github.com/agentscope-ai/QwenPaw)** | v2.0.0 July 10: "Agent OS" architecture — Resources, Governance, kernel-level sandbox (Tool Guard + File Guard). Three-layer memory (working/history/distilled); bundled local models (2B/4B/9B); 14+ IM platform connectors. Second "Agent OS" signal after ouroboros (2026-05-04); different governance mechanism (kernel sandbox vs spec-first). Apache 2.0. | L2 |
| **Qwen3-Coder-Next (Alibaba)** | 70.6% SWE-Bench Verified — highest tracked open-weight coding score. MoE: 30B total / 3B active per token. 256K context. RL-trained on agentic coding tasks. Apache 2.0 open weights. AWS Bedrock listing provides institutional validation. Pricing unconfirmed (Bedrock pricing page lookup needed). Closest to registry-ready of today's signals. | L1 pending |
| **[tirth8205/code-review-graph ⭐20.9k](https://github.com/tirth8205/code-review-graph)** | Second consecutive day on GitHub Trending (now +551 today). 30 MCP tools, 82x token reduction claim, GitHub Action merge gates. L4c code-intelligence cluster leader. Two-signal watch still active — independent signal needed for canonical L4c promotion. | L4c |
| **Kimi K3 (MoonshotAI)** | 2.8T MoE, 1M token context, $3/$15 per 1M. Self-reported FrontierSWE 81.2%. Weights expected 2026-07-27. Moonshot AI suspended new subscriptions due to demand surge (July 18). Now in direct competition with Qwen3.8 (announced same day). | L1 hold |
| **[lobehub/lobe-chat ⭐80.4k](https://github.com/lobehub/lobe-chat)** | "Chief Agent Operator" platform: agent team scheduling, 10,000+ MCP Skills, White-Box Memory. Largest open-source multi-agent orchestration platform. v2.2.10, 2,850+ releases. | L2 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (487 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
