...
## 🔥 What's hot right now (2026-07-09)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[agno-agi/agno](https://github.com/agno-agi/agno) ⭐26k** | Multimodal agent runtime with MCP interface v2 (8-tool operator surface) — shipped two releases this week (v2.7.0 July 7, v2.7.2 July 9): eval suite runner, PAT service accounts, OAuth hardening, AG-UI client tools. First tracked L1 runtime that is also a versioned MCP capability provider, not just a consumer. | L1/L4c |
| **[vxcontrol/pentagi](https://github.com/vxcontrol/pentagi) ⭐19.3k** | Fully autonomous AI agent for complex penetration testing — no per-action human confirmation gate; scope authorization is the only human input. First tracked L1 runtime using an "authorization-scoped" autonomy model (vs. approval-gated or goal-directed). Go single-binary; enterprise security org provenance. | L1/L2 |
| **[ruvnet/RuView](https://github.com/ruvnet/RuView) ⭐78.3k** | WiFi spatial AI (ESP32 mesh): presence, vitals, 17-keypoint pose — MCP server (`rvagent`) exposes physical environment state to Claude, Cursor, and swarm agents. First "physical-world MCP capability" signal; 78k stars reflects maker + agent ecosystem overlap. | L4c |
| **[microsoft/SkillOpt](https://github.com/microsoft/SkillOpt) ⭐11.9k** | Text-space optimizer for training reusable language skills — optimizes skill definitions for cross-model portability, not single-model performance. Part of coherent Microsoft skills-layer push (agent-framework v1.13 added `AgentSkillsSourceContext` July 3). First tracked tool with portability as the optimization target. | L4a |
| **[LMCache/LMCache](https://github.com/LMCache/LMCache) ⭐10.3k** | Provider-agnostic KV cache layer: drop-in between LLM clients and any OpenAI-compatible server; eliminates redundant computation on repeated system prompts and shared context. Lossless counterpart to pxpipe (tracked 2026-07-05); directly relevant to clawfit's cost and latency filter accuracy for multi-turn workflows. | L7 |
| **[wonderwhy-er/DesktopCommanderMCP](https://github.com/wonderwhy-er/DesktopCommanderMCP) ⭐6.5k** | MCP server granting Claude terminal command execution and filesystem read/write via standard MCP tool calls. Clean L4c reference for "local OS access" sub-type — any MCP-compatible runtime can consume it without agent-specific integration. Likely affected by MCP 2026-07-28 RC stateless spec (persistent process state). | L4c |
| **[HKUDS/AI-Trader](https://github.com/HKUDS/AI-Trader) ⭐20.6k** | Agent-native trading environment: agents register, copy-trade each other's strategies, and compete on a $100K leaderboard. Not "agents executing trades for humans" — agents trading against each other. Second HKUDS financial-domain signal. | L1/L5 |
| **[langbot-app/LangBot](https://github.com/langbot-app/LangBot) ⭐16.7k** | Multi-platform messaging agent framework — 11 platforms (Discord, Slack, Telegram, WeChat, WeCom, QQ, Lark, LINE, DingTalk, KOOK, Matrix) in one runtime; native MCP; v4.10.5, 199 releases. First dedicated messaging-native L1 framework tracked. | L1/L2 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (416 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
