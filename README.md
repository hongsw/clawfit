...
## 🔥 What's hot right now (2026-07-28)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **Bun Zig→Rust via 64 Claude Code instances** | 535,496 lines migrated in 11 days using 64 parallel Claude instances, ~50 dynamic workflows, $165k compute. 100% CI pass before merge; memory 6.7 GB → 609 MB. First mega-scale AI-assisted migration case study. Counter-signal: Zig creator called output "unreviewed slop" — CI pass ≠ idiomatic quality. | L2 use-case |
| **[HKUDS/OpenSpace ⭐7,137](https://github.com/HKUDS/OpenSpace)** | Skill management layer for AI agents: dynamic retrieval, execution outcome tracking, and three controlled evolution operations (FIX/DERIVED/CAPTURED). First "feedback-loop skill system" signal — all prior L4b signals treat skills as static artifacts. 866 forks (unusually high ratio). Schema gap: `skill_evolution`. | L4b / L5 |
| **Kimi Linear attention (HN 211 pts)** | New efficient attention architecture from 60+ MoonshotAI researchers (underlying Kimi K3). KDA + MLA hybrid; 75% KV cache reduction; 6× decoding throughput at 1M-token context. If reproduced, changes hardware-tier threshold for offline 1M-context agents. Schema gap: `attention_type`. | L1 |
| **Anthropic open-weights position (HN #1, 322 pts)** | Anthropic officially endorses open-weights models as "beneficial public goods." First first-party frontier-lab open-weight endorsement. Three policy commitments: chip-sale restrictions, distillation targeting, mandatory safety testing. Validates hybrid/offline track in clawfit recommendations. | policy |
| **[vudovn/ag-kit ⭐7,950](https://github.com/vudovn/ag-kit)** | TypeScript harness for Google Antigravity platform: 20 specialist agents, 47 reusable skills, 13 slash-command workflows, persistent memory, MCP, native safety hook. First signal for a Google-AI-first harness; first harness explicitly optimized for a Google-family LLM backend. Schema gap: `primary_llm_target`. | L2 |
| **[mastra-ai/mastra ⭐26.6k](https://github.com/mastra-ai/mastra)** | TypeScript-native AI agent framework with 40+ provider routing, graph-based workflow engine, three-tier memory, and MCP server hosting. 17k+ commits, Apache 2.0 core. First dedicated signal; fills a gap — clawfit registry has no TypeScript-native L2 entry. Schema gap: `stack_language`. | L2 |
| **Microsoft MAI-Cyber-1-Flash + MDASH** | Security-specialized LLM inside MDASH (multi-agent vulnerability identification harness). 95.95% CyberGym, claims 50% cost vs. leading models. Production domain-specialized model + cost-tiered escalation routing in enterprise multi-agent deployment. | L1/L2 |
| **[can1357/oh-my-pi ⭐19.7k](https://github.com/can1357/oh-my-pi)** | Terminal coding agent with embedded LSP + DAP + MCP as first-class tool primitives. Native Rust engine, 32 built-in tools, 40+ LLM backends, ~400 releases since Dec 2025. High-signal recurring reference for full-featured L2 production agents. | L2 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (533 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
