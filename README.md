...
## 🔥 What's hot right now (2026-08-02)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **Karpathy "Pelican" tweet — Opus 5 2-hour $10 creative session (HN #4, 119 pts)** | Andrej Karpathy (Anthropic) uses the "pelican on a bicycle" SVG benchmark as a before-marker; demonstrates Claude Opus 5 generating 5,500 lines of three.js from a Lord of the Rings paragraph in ~2 hours at ~$10 (1M tokens) as the after-marker. Claim: frontier models have advanced past the task class that benchmark was designed to measure. First data point anchoring creative/procedural coding at the total-session cost level rather than per-token rate. | Ecosystem calibration signal |
| **[NomaDamas/k-skill ⭐6,726](https://github.com/NomaDamas/k-skill) (GitHub Trending)** | Korean-language skill collection for AI agents (Claude Code, Cursor, Cline) implemented in JavaScript. Pure skill-manifest format, no MCP dependency. Targets Korean-language workflows: document summarization, legal research, Korean API connectors. First geographically-scoped L4b skill pack in the corpus — prior entries are domain-specialized, not locale-specialized. Schema gap: `locale: [global | ko | ja | zh | ...]`. | L4b locale skill pack |
| **Cursor removed cost info from usage dashboard (HN 293 pts, 127 comments)** | Cursor silently removed per-token cost breakdowns and token amounts from usage dashboard and CSV export. High HN engagement surfaces the tension between commercial AI tool opacity and developer cost observability. First production example of "proprietary agent cost opacity" pattern. Scoring implication: `monthly_budget` filter is unreliable for proprietary closed IDEs with non-transparent billing. | Cross-cutting governance |
| **[VideoLingo ⭐18,016](https://github.com/Huanshere/VideoLingo) (GitHub Trending Python, +48 stars)** | End-to-end video localization pipeline: download → WhisperX transcription → NLP segmentation → three-stage LLM loop → TTS dubbing. Translate-Reflect-Adapt LLM chain (translate → critique → culturally adapt) applies write-review-revise from coding workflows to natural language translation. Model-agnostic: Claude Sonnet, GPT-4, Gemini, DeepSeek, Grok. Recency caveat: v3.0.1 was February 2025; no confirmed major 2026 release. | L4 video localization |
| **[tradingview-mcp ⭐3,756](https://github.com/atilaahmettaner/tradingview-mcp) (GitHub Trending Python)** | Self-hostable MCP server exposing 37 financial market data tools — backtesting (with walk-forward overfitting detection), price feeds, technical analysis, screeners, Reddit+RSS sentiment — to any MCP-compatible agent with no credentials required. Second signal for the "live-data-connector MCP server" pattern (first: SurfSense 2026-07-25, web/social domain). Two-signal cross-day pair for this pattern. | L4 MCP financial data |
| **Cerebras Knowledge — enterprise internal KB (GeekNews 45 pts × 3 days)** | Third consecutive GeekNews appearance for Cerebras's internal knowledge base integrating Slack, code repos, docs, and databases. No public repo — internal system. Pattern signal: enterprise-scale knowledge aggregation for AI agents is a validated operational need at production companies, not speculative. Calibration signal for L4a tools: `team_size: solo/small` ceilings may be underspecified. | L4a enterprise memory signal |
| **[yc-software/qm](https://github.com/yc-software/qm) (HN 634 pts)** | YC-backed multiplayer agent harness for teams: dual Slack + web interface, isolated per-employee workspaces with shared skills, three governance postures (Strict/Auto/Dangerous), background cron jobs. First team-scope harness with native approval-workflow governance. | L2 team harness |
| **[zhaoxuya520/reverse-skill ⭐11,681](https://github.com/zhaoxuya520/reverse-skill) (GitHub Trending #4 yesterday)** | AI-powered skill router for authorized reverse engineering and penetration testing. Router architecture with self-evolving KB claim, 15.2% fork ratio — highest in the security skill cluster. Fourth security-domain agent skills signal. | L4b security skill router |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (561 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
