...
## 🔥 What's hot right now (2026-07-16)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[Inkling — Thinking Machines Lab](https://thinkingmachines.ai/news/introducing-inkling/)** | 975B-parameter open-weights MoE (41B active) trained on 45T tokens of text, image, audio, and video. 1M-token context, controllable thinking effort, 30M+ RL rollouts for log-linear reasoning. Fine-tunable via Tinker platform. Highest HN-points signal today (583 pts). Open weights make it viable for `data_sensitivity: confidential` and `governance_need: hard` profiles that cannot use closed APIs. | L1 LLM |
| **[xai-org/grok-build](https://github.com/xai-org/grok-build)** | xAI open-sources Grok Build: a Rust-based full-screen TUI coding agent with headless CI/CD mode and ACP (Agent Client Protocol) support — second tracked L1 runtime to implement ACP natively alongside OpenInterpreter Rust. Direct competitor to Claude Code, Goose, Crush, and Aider. Added to registry. | L1 |
| **[Ambiance — Filesystem Agent Harness](https://eardatasci.github.io/c/ambiance/index.html)** | Harness where filesystem (FHS-inspired) is the shared-state bus: file-change events trigger specialized LLM agents (`root`, `pai`, `librarian`) with no polling. Third independent harness-engineering signal in four days — triggered canonical promotion of `harness_trigger` and `agent_communication` fields to L2 axis. | L2 |
| **[Codex MultiAgentV2 — Audit Trail vs. Encryption](https://github.com/openai/codex/issues/28058)** | OpenAI Codex now encrypts inter-agent payloads, making child-agent task assignments opaque to operators. First deployment-scale instance of the audit-trail vs. confidentiality tradeoff. Signals that tools encrypting inter-agent comms without operator-accessible audit logs should score lower on `governance_need: hard` profiles. `audit_trail` schema candidate. | Governance |
| **[openinterpreter/openinterpreter ⭐65.3k](https://github.com/openinterpreter/openinterpreter)** | Complete Rust rewrite; named harness emulation modes (`claude-code`, `deepseek-tui`, `qwen-code`) allow behavioral switching against any backend. Native sandboxing, ACP editor integration. First tracked runtime with `harness_mode` as a runtime configuration primitive. | L1 |
| **[HKUDS/nanobot ⭐45.7k](https://github.com/HKUDS/nanobot)** | Single agent loop serving Telegram, Discord, Slack, WeChat, Feishu, Mattermost, and email via WebSocket gateway. Multi-provider LLM, "Dream" persistent memory, native MCP. Introduces `statefulness: persistent-cross-channel` as a new tier candidate. | L2 |
| **["Designing APIs for Agents"](https://www.freestyle.sh/blog/opinion/designing-apis-for-agents)** | Agent-centric API design: explicit-over-implicit parameters, unambiguous naming, precise errors as reasoning-trace teaching moments. As LLM agents become primary API consumers, design conventions must diverge from human-developer ergonomics. Applies to clawfit's own CLI output format. | Ecosystem |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (467 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
