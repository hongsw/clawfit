...
## 🔥 What's hot right now (2026-07-25)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[can1357/oh-my-pi ⭐19.7k](https://github.com/can1357/oh-my-pi)** | Terminal coding agent with embedded LSP + DAP + MCP as first-class tool primitives — not subprocess wrappers. Native Rust engine (~55k lines), 32 built-in tools, 40+ LLM backends, content-hash-anchored edits preventing stale-read corruption under concurrent multi-agent editing. ~400 releases since Dec 2025. Second signal for oh-my-pi; first dedicated assessment as a full L2 production agent. | L2 |
| **[MODSetter/SurfSense ⭐15.5k](https://github.com/MODSetter/SurfSense)** | Open-source NotebookLM alternative that exposes 10+ live social + web scrapers (Reddit, YouTube, TikTok, Google Search/Maps, Amazon, etc.) simultaneously as REST endpoints and native MCP tools. Agents call `surf_reddit` or `surf_web` as first-class tool calls — no user copy-paste. First signal for "live-data-connector MCP server" sub-type. | L4c/L5 |
| **UK AISI/Caisi Kimi K3 Cyber Assessment** | First publicly available government safety institute evaluation (UK AISI + Caisi, NIST-hosted, 114 HN pts) of a production frontier coding model's offensive cybersecurity capabilities. Kimi K3 holds the current frontier position (FrontierSWE 81.2%); this assessment adds a disclosure dimension for `governance_need: hard` profiles. First signal for government safety assessment as an L1 model annotation type. | L1 gov. |
| **NousResearch/hermes-agent v0.19.0 "Quicksilver"** | ~80% drop in first-turn TTFT (harness-layer streaming pipeline, applies across all 40+ LLM backends) + live reasoning streams now default-on. If TTFT claim verified, Hermes may reclassify from `latency: medium` to `latency: low` — unlocking profiles where it's currently filter-eliminated. Sixth signal; first to surface an L6 interface-transparency dimension. | L1/L6 |
| **[OpenDCAI/DataFlow ⭐6.97k](https://github.com/OpenDCAI/DataFlow)** | LLM-powered data preparation pipeline for SFT datasets and RAG knowledge bases. DataFlow Agent assembles pipeline DAGs from natural language prompts — the second ecosystem signal for "LLM assembles task DAG from goal description" (first: open-multi-agent 2026-07-24). Ray-backed distributed execution; OpenAI + vLLM/SGLang backends. Apache 2.0. | L5 |
| **Claude Opus 5 (Anthropic)** | Frontier reasoning model ($15/$75 per 1M tokens, 200k context) added to `llms.json` 2026-07-24. Tops SWE-bench Verified (72.5%) and GPQA Diamond (78.0%). The highest-scoring entry in the current clawfit registry for `task: research` and `task: code-gen` profiles. | L1 LLM |
| **[open-multi-agent ⭐6.7k](https://github.com/open-multi-agent/open-multi-agent)** | TypeScript multi-agent framework where a coordinator LLM constructs the task DAG at runtime — no hand-drawn graphs required. Deterministic scheduler executes once the plan is approved; checkpoint-based recovery resumes from last success. Token budget enforcement; loop detection; multi-model teams (Claude, GPT, Gemini). First signal for runtime DAG planning as distinct L2 sub-type. | L2 |
| **[humanlayer/acp](https://github.com/humanlayer/acp)** | Kubernetes-native agent control plane: agents, tools, tasks, and tool-calls modeled as CRDs — visible via `kubectl`, alertable via standard cluster monitoring. Durable checkpoint-resume execution; human-approval gates as first-class tool types; MCP-compatible; OpenTelemetry built in. First L2 harness requiring cluster infrastructure, not a developer laptop. | L2/L5 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (481 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
