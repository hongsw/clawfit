...
## 🔥 What's hot right now (2026-08-04)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **["LLMs reward expertise" (seangoedecke.com)](https://www.seangoedecke.com/llms-reward-expertise/) (HN #1 1,273 pts, 2026-08-04)** | LLMs amplify the expert advantage rather than closing skill gaps. Experts make fewer follow-ups, push back specifically on wrong outputs, and achieve dramatically higher leverage — non-experts cluster near the floor of the productivity distribution. Pairs with "2x not 10x" (July 31): together they frame LLM productivity as bimodal, not uniform. Schema gap: `operator_expertise_level`. | Cross-cutting calibration |
...
| **[leonickson1/Swiftlet ⭐317](https://github.com/leonickson1/Swiftlet) (HN Show HN 285 pts, 2026-08-04)** | Swift + Metal runtime running 80B Qwen in 4.3 GB RAM on Mac and 35B on iPhone 17 (~1 tok/s) via MoE expert weight streaming from SSD. Third independent implementation of the SSD-streamed MoE inference pattern (turbo-fieldfare July 29, WASTE August 1, Swiftlet today) — pattern confirmed across Apple Silicon, x86/NVMe, and iOS. First signal for iOS as a viable agent inference tier. | L7/L1 iOS inference runtime |
...
| **[garagehq/nightcrawler ⭐293](https://github.com/garagehq/nightcrawler) (HN Show HN 86 pts, 2026-08-03)** | Autonomous red-team agent running entirely on Android using on-device LLM inference (LFM2.5-1.2B on phone GPU). No outbound cloud LLM calls. Bundled CVE database + attack playbooks make sub-2B reasoning viable for structured security tasks. First signal for on-device mobile GPU as an agent inference tier and for `governance_need: operational-stealth` (stealth vs. compliance). | L1/L7 on-device mobile security |
| **[micro/mu ⭐112](https://github.com/micro/mu) (HN Show HN, 2026-08-03)** | Single Go binary bundling 67 tools (web search, news, weather, markets, mail, storage, calendar, image gen) behind one MCP config entry. Real infrastructure per domain — not thin API wrappers. Solves "too many MCP servers" at the aggregator layer. Pairs same-day with graph-tool-call (router-based mitigation): two independent solutions to the MCP tool saturation failure mode. Registry entry added. | L4c MCP mega-aggregator |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (573 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
