...
## 🔥 What's hot right now (2026-07-18)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[tirth8205/code-review-graph ⭐20k](https://github.com/tirth8205/code-review-graph)** | Local-first code intelligence graph for MCP: 30 tools (impact analysis, blast-radius tracing, semantic search, change detection), 82x median token reduction claim, GitHub Action merge gates. GitHub Trending rank 9 all languages. MIT, Python 94%, v2.3.6. Broadest tool count in the L4c code-intelligence cluster; two-signal watch active for canonical L4c promotion. | L4c |
| **Kimi K3 (MoonshotAI)** | 2.8T-parameter MoE (16/896 experts active), 1M token context, $3/$15 per 1M input/output. Self-reported FrontierSWE 81.2%, Terminal-Bench 88.3%. Closes the 256K context gap from K2.6. Regression: K3 drops K2.6's dual Thinking/Instant modes — max-effort reasoning only, not a `latency: low` candidate. Weights expected 2026-07-27. | L1 LLM |
| **State of Open Source AI V1.0** | Mozilla-backed survey of the open-weight ecosystem (9 layers, 48 components). Key findings: capability gap collapsed then reopened (3.3% vs closed reasoning); inference cost $20→$0.40/1M tokens over 36 months; open deployment lag (51% vs 63% closed) explained by compliance/infrastructure complexity, not capability. HN 353 pts, July 18. | Meta |
| **[MoonshotAI/kimi-cli ⭐9.4k](https://github.com/MoonshotAI/kimi-cli)** | Terminal coding agent from MoonshotAI (same lab as K3) — vertical integration pattern matching Anthropic (Claude + Claude Code) and xAI (Grok + Grok Build). Full MCP client + Agent Client Protocol (ACP) for Zed/JetBrains. Being deprecated in favor of `kimi-code`. Apache 2.0. | L1 |
| **[OpenSenseNova/SenseNova-U1 ⭐3.96k](https://github.com/OpenSenseNova/SenseNova-U1)** | Native unified multimodal model (SenseTime): NEO-unify eliminates both the visual encoder (CLIP/SigLIP) and VAE, treating image pixels and text as a single natively intermixed token stream. 8B dense + A3B MoE variants. Apache 2.0. Exposes schema gap in clawfit — `llms.json` fields (cost_per_1k_tokens, tasks) are incompatible with pixel-output models. | L1 MM |
| **[KnockOutEZ/wigolo ⭐1.1k](https://github.com/KnockOutEZ/wigolo)** | Local-first web intelligence MCP server: 10 tools (search, fetch, crawl, research, agent, diff, watch), 18 direct engine adapters, ML reranking, $0/query. Fills the `network: offline` + `task: research` gap — cloud search MCP tools don't work without internet. AGPL-3.0; ~1.5 GB on-device. Public beta. | L4c |
| **[lobehub/lobe-chat ⭐80.4k](https://github.com/lobehub/lobe-chat)** | "Chief Agent Operator" platform: hires, schedules, and reports on AI agent teams in 7×24 persistent operations. Four-pillar architecture — Operator, Create (10,000+ MCP Skills), Collaborate (Groups + Scheduling), Evolve (White-Box Memory). Mature: 2,850 releases, v2.2.10 July 10. Largest open-source user base in the multi-agent orchestration tier. | L2 |
| **[xai-org/grok-build](https://github.com/xai-org/grok-build)** | xAI open-sources Grok Build: Rust-based full-screen TUI coding agent with headless CI/CD mode and ACP support — second tracked L1 runtime to implement ACP natively. Added to registry. | L1 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (479 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
