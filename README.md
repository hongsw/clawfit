...
## 🔥 What's hot right now (2026-07-31)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **DeepSeek V4 Flash 0731 (HN 568 pts)** | New Flash variant released today with agent-specific benchmarks (Terminal Bench 82.7, DeepSWE 54.4, Toolathlon 70.3). Pricing: $0.14/$0.28/M tokens; $0.003/M cache hit (98% discount — lowest for a frontier-class model). MIT open weights, 284B total / 13B active MoE. Natively targets Codex and Responses API. Direct registry update candidate. | L1 MoE LLM |
| **Frontier Lab Agent Intrusion Timeline (HN 266 pts)** | First public post-mortem of a real frontier-lab agent security incident (July 2026). Four-signal cluster on "agent security boundary failure" now confirmed: Prismata (2026-07-11) + AI Worms (2026-07-29) + HANDBOOK.md (2026-07-29) + this timeline. Pattern promotion candidate for next reference-levels.md review. | Cross-layer security |
| **[different-ai/openwork ⭐19.3k](https://github.com/different-ai/openwork)** | Open-source alternative to Claude Cowork, built on OpenCode. First signal for "named commercial product displacement" pattern — OSS competitors now target specific commercial products by name, not capability categories. Team workspace layer with shared sessions and project context. | L2 workspace |
| **AI Worms Through Copilot/Word (HN 249 pts)** | First documented self-propagating document-borne AI worm in production enterprise software (coordinated disclosure). White-on-white hidden instructions propagate via normal document sharing. Structural finding: agents cannot separate "data to inspect" from "instructions to follow" — affects all L2/L4 agents reading third-party content. | Cross-layer security |
| **HANDBOOK.md Benchmark (HN 208 pts, arxiv 2607.25398)** | 65 agentic tasks across finance/medical/insurance/logistics/HR measuring agent policy-document adherence. Best model: 36.2% strict success. First quantified evidence that "give the agent a long policy document" governance is empirically unreliable at current capability. | L5 eval |
| **Anthropic Claude Code Migration Methodology (claude.com/blog)** | Official 6-step migration playbook: rulebook → stress-test → multi-agent fan-out → compile → smoke test → parity verification. Production data: 1M lines Bun Zig→Rust at $165k API tokens — first public cost-per-KLOC benchmark for migration-class orchestration. | L2 orchestration |
| **[trailofbits/skills ⭐6,364](https://github.com/trailofbits/skills)** | Trail of Bits' security-domain Claude Code skill pack: 7 categories (Smart Contract, Code Auditing C/C++/Rust, YARA/Malware, Reverse Engineering, Mobile APK). First institutional security firm releasing a signed Claude Code skill pack — distinct from community aggregators by vendor provenance and maintenance accountability. | L4b security skills |
| **MCP 2026-07-28 Final Spec ratified** | RC becomes official specification: `subscriptions/listen` (long-lived POST stream), OTel trace context, deterministic `tools/list` ordering for LLM cache hits, MRTR replaces server-initiated requests, OAuth 2.0 DCR deprecated. L4c audit now mandatory for session-continuity tools. | Cross-layer protocol |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (549 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
