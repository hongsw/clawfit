...
## 🔥 What's hot right now (2026-07-22)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[bojieli/ai-agent-book ⭐14,349](https://github.com/bojieli/ai-agent-book)** | Open-source agent engineering textbook ranked #2 on all-language GitHub Trending 2026-07-22. 10 chapters + 88 executable projects cover the full taxonomy: context engineering (L5), tools (L4), coding agents (L2), evaluation, model post-training, multi-agent collaboration. "Agent = LLM + Context + Tools" organizing formula. Multi-language from launch (EN/ZH/ZH-TW/Tamil/Vietnamese). Star count is a discipline-maturation signal, not just tool adoption. | L1–L5 |
| **[earthtojake/text-to-cad ⭐9,090](https://github.com/earthtojake/text-to-cad)** | First physical-world domain skill pack in the scan corpus. 10 skill categories: CAD (STEP/STL/3MF/GLB), DXF 2D drawings, URDF/SRDF robot structures, G-code with real slicer CLI integration, Bambu Labs 3D printer control, SendCutSend fabrication validation. Built on Build123d + OpenCASCADE — production-grade geometry kernels, not toy shapes. 1k forks, active Discord. | L4b |
| **[ayghri/i-have-adhd ⭐6,811](https://github.com/ayghri/i-have-adhd)** | 10-rule output discipline skill: lead with action, number steps, no preamble, no recap, cap lists at 5 items. Packaged as a cross-agent plugin (`plugin.json`, `.claude-plugin/`, `.codex-plugin/`). First installable output-discipline skill — confirms agent behavioral constraints are becoming distributable marketplace artifacts. | L4b |
| **[block/buzz (213 HN pts)](https://github.com/block/buzz)** | Block's open-source team workspace where AI agents receive Nostr Ed25519 keypairs and participate as full members using the same primitives as humans — no secondary bot API. Agent-as-first-class-principal identity model encoded at the protocol layer, not in application policy. Apache 2.0, self-hostable, v0.4.21. | L6/L4 |
| **[poolside/Laguna-S-2.1](https://huggingface.co/poolside/Laguna-S-2.1)** | 118B-total / 8B-active MoE coding model (256 routed experts, top-10). RL post-training ran across multiple harnesses to prevent eval overfitting. Ships with `pool` harness (terminal coding agent, up to 500 steps/task, sandboxed) — model-plus-harness pair, not a standalone drop. $0.10/$0.20 per 1M tokens on OpenRouter, 1M-token context. | L1 |
| **[JustVugg/colibri ⭐17,430](https://github.com/JustVugg/colibri)** | Pure-C, zero-dep MoE inference engine running GLM-5.2 (744B) on 25GB RAM by streaming expert weights from disk. Learning cache; 71.6% prefetch predictability. v1.0.0. **Two-signal "MoE-native consumer inference" confirmed** with ktransformers (2026-07-19). | L1 |
| **[omnigent-ai/omnigent ⭐7,576](https://github.com/omnigent-ai/omnigent)** | Meta-harness orchestrating Claude Code, Codex, Cursor, OpenCode, Hermes, and Pi under YAML-defined session model. Supervisor pattern native; cross-device sync; governance layer (command approval, per-agent spend caps). **Two-signal "meta-harness for multi-agent coding" confirmed** with loop-engineering (2026-07-05). | L2/L3 |
...
Full analysis in [`docs/research-watch/`](docs/research-watch/) (503 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
