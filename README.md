...
## 🔥 What's hot right now (2026-08-06)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[Discovery Loop](https://discoveryloop.com) (HN 536 pts, 2026-08-06)** | Jeff Dean, Sanjay Ghemawat, Quoc Le, and Oriol Vinyals departed Google August 5, 2026 to co-found a startup automating the scientific experimental loop (propose → execute → learn) at scale, running thousands of experiments simultaneously. Seed-funded by Radical Ventures + Khosla Ventures; Google as founding investor + Cloud partner. Highest-credential founding team in the corpus — and the most direct embodiment of L5 (automated research loop) seen to date. Pre-product. | L5 automated research loop |
| **[Atlassian Rovo prompt injection](https://www.promptarmor.com/resources/atlassian-rovo-exfiltrates-data) (HN 155 pts, 2026-08-06)** | PromptArmor disclosed a zero-click indirect prompt injection in Atlassian Rovo that bypasses org-level web search controls, exfiltrating any data accessible via Rovo's connectors (Jira, Confluence). No user action required. A separate one-click URL-parameter variant was patched; the indirect injection via URL retrieval tool remains an architectural concern. First documented real-world enterprise breach via indirect prompt injection in the corpus. | L3 enterprise agent security signal |
| **[google-gemini/gemini-cli ⭐106,400](https://github.com/google-gemini/gemini-cli) (GitHub Trending, 2026-08-06)** | Google's official open-source terminal agent: Gemini 3 models with 1M-token context window, native Google Search grounding as a built-in tool (not MCP-pluggable), MCP client support, conversation checkpointing, GitHub Actions integration for PR reviews. Free tier: 60 req/min, 1,000 req/day. Direct competitor to Claude Code, Codex CLI, and Meta Muse Code. First formal tracking of Google's terminal agent — a significant gap given 106k stars. Two-signal cluster with Meta Muse Code (both hyperscaler-native terminal agents confirmed today). | L1 Google-native terminal agent |
| **[Meta Muse Code](https://research.meta.ai/blog/introducing-muse-code-and-muse-spark-1-2) (HN 145 pts, 2026-08-06)** | Meta Superintelligence Labs terminal coding agent (public beta Aug 5): Muse Spark 1.2 model, 1M-token context, persistent async background agents (parallel workers + background reviewers per task). Structurally novel: contributor pricing tier offers lower rates in exchange for training data contribution — a `data_sensitivity` scoring dimension not present in Claude Code or Codex. | L1 Meta-native terminal agent |
| **[Zed DeltaDB](https://zed.dev/deltadb) (HN 267 pts, 2026-08-06)** | CRDT-based version control from Zed Industries recording every sub-commit editing operation and permanently linking each change to the agent conversation that produced it. Bidirectional tracing: code → conversation, conversation → code. Multiple agents co-edit without merge conflicts. First tool treating the agent conversation as a first-class audit artifact in the version control layer — a "live pre-commit layer" alongside Git. | L2/L3 agent-native CRDT version control |
| **[Lilian Weng "Harness Engineering for Self-Improvement"](https://lilianweng.github.io/posts/2026-07-04-harness/) (GeekNews front page, 2026-08-06)** | 35-paper synthesis by former OpenAI Safety Head (now Lilith AI CEO) arguing the near-term path to recursive self-improvement runs through harness engineering — not weight modification. Seven open challenges enumerated. Endorsed by DeepSeek CTO Cui Tianyi. Most rigorous single-source articulation of why L2 harness engineering matters; provides theoretical grounding for loopx, cloudflare/computer, Ornith-1, and Evolver GEP as a coherent research direction. | L2 harness engineering conceptual framework |
| **[cloudflare/cloudflare-os ⭐1,400](https://github.com/cloudflare/cloudflare-os) (HN 317 pts, 2026-08-05)** | Cloudflare's internal enterprise agent workspace, open-sourced. Agents access external systems only through Gatekeepers (Workers mediating access, holding credentials — agents never hold credentials directly). Observation-tracking enforces downstream permissions on data derived from confidential sources. Most sophisticated data-lineage enforcement primitive in the corpus. MCP-compatible; model-agnostic via AI Gateway. | L2/L3 enterprise agent workspace |
| **[Gen-Verse/Open-AgentRL ⭐607](https://github.com/Gen-Verse/Open-AgentRL) (GitHub, ICML 2026, 2026-08-06)** | ICML 2026-accepted bundle: RLAnything (joint closed-loop RL optimization across all components), AutoTool (dynamic agent tool selection from thousands of tools), DemyAgent (4B model matching 32B via real trajectory datasets). Third independent signal in the "RL training for agentic tool-use" cluster (OpenClaw-RL 07-05, SkyRL 08-05, Open-AgentRL 08-06). | L1 RL training for agentic tool-use |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (585 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
