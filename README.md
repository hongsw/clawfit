...
> Agent + LLM + hardware recommendation engine — **162+ tools mapped**, **561 research-watch docs**, daily automated scanning.
...
## 🔥 What's hot right now (2026-08-03)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[callstack/agent-device ⭐3,900](https://github.com/callstack/agent-device) (GeekNews, 2026-08-03)** | CLI that gives AI agents control of iOS, Android, tvOS, Flutter, and React Native UIs via native accessibility snapshots — no screenshot OCR, no Appium wrapper. Structural analog to browser-use (web DOM) but for mobile. Built by Callstack (core React Native maintainers). Exports agent runs as CI/CD test scripts. First signal for mobile device control as an L6 computer-use sub-type. Schema gap: `agent_control_target: mobile-ios/android`. | L6 mobile computer-use |
| **[garagehq/nightcrawler ⭐293](https://github.com/garagehq/nightcrawler) (HN Show HN 86 pts, 2026-08-03)** | Autonomous red-team agent running entirely on Android using on-device LLM inference (LFM2.5-1.2B on phone GPU). Full lifecycle: recon → enumeration → exploitation → report, with no outbound cloud LLM calls. Bundled CVE database + attack playbooks make sub-2B reasoning viable for structured security tasks. 86 HN points despite security category friction. First signal for on-device mobile GPU as an agent inference tier. | L1/L7 on-device security |
| **[micro/mu ⭐112](https://github.com/micro/mu) (HN Show HN, 2026-08-03)** | Single Go binary bundling 67 tools — web search, news, weather, markets, mail, storage, calendar, image gen — behind one MCP config entry. Real infrastructure per domain (own SMTP server with DKIM, search index, app sandbox) rather than thin API wrappers. Solves the "too many MCP servers" problem at the aggregator layer. First "mega-aggregator MCP endpoint" signal. Registry entry added. | L4c MCP aggregator |
| **graph-tool-call (SonAIengine — GeekNews, 2026-08-03)** | Developer documents a repeatable accuracy degradation when connecting 5+ MCP servers simultaneously (GitHub + Slack + DB + browser + internal API). Built `graph-tool-call` as a graph-based routing layer to narrow the tool selection space before the model sees it. Same-day pair with Mu: two distinct mitigations (aggregator vs. router) for the same MCP tool-count failure mode. First named "MCP tool saturation" pattern. | Cross-cutting harness |
| **[graham33/nixos-dgx-spark](https://github.com/graham33/nixos-dgx-spark) (HN 88 pts, 2026-08-03)** | NixOS running on NVIDIA DGX Spark (GB10 Grace Blackwell, ~1 PFLOP, ~$3,000): declarative OS + driver stack + inference runtime pinned in flake.nix. New hardware tier: above consumer-desktop (RTX Spark) and below cluster-scale. NixOS angle introduces `deployment_model: nix-reproducible` — fully reproducible air-gapped AI deployments. | L7 hardware tier |
| **[NomaDamas/k-skill ⭐6,726](https://github.com/NomaDamas/k-skill) (GitHub Trending)** | Korean-language skill collection for AI agents (Claude Code, Cursor, Cline). Pure skill-manifest format, no MCP dependency. First geographically-scoped L4b skill pack in the corpus — prior entries are domain-specialized, not locale-specialized. Schema gap: `locale: [global | ko | ja | zh | ...]`. | L4b locale skill pack |
| **[tradingview-mcp ⭐3,756](https://github.com/atilaahmettaner/tradingview-mcp) (GitHub Trending Python)** | Self-hostable MCP server exposing 37 financial market data tools — backtesting (with walk-forward overfitting detection), price feeds, technical analysis, screeners, Reddit+RSS sentiment — to any MCP-compatible agent with no credentials required. Second signal for "live-data-connector MCP server" pattern. | L4 MCP financial data |
| **Cursor removed cost info from usage dashboard (HN 293 pts, 127 comments)** | Cursor silently removed per-token cost breakdowns and token amounts from usage dashboard and CSV export. First production example of "proprietary agent cost opacity" pattern. Scoring implication: `monthly_budget` filter is unreliable for proprietary closed IDEs with non-transparent billing. | Cross-cutting governance |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (566 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
