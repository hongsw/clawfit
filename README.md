...
## 🔥 What's hot right now (2026-07-14)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[arXiv 2607.01418 — Microsoft adoption study](https://arxiv.org/abs/2607.01418)** | First peer-reviewed enterprise field study on agentic CLI tools: +24% PR merge rate for Claude Code / Copilot CLI adopters vs. non-adopters across tens of thousands of Microsoft engineers. Adoption spreads via social networks; retention correlates with coding activity, not demographics. Empirical grounding for `team_size: large` scoring. | Ecosystem |
| **["Own the Outer Loop" — Addy Osmani](https://addyo.substack.com/p/own-the-outer-loop)** | Author of `addyosmani/agent-skills` ⭐76k frames the human-agent boundary: agents own the inner loop (implementation), engineers must own the outer loop (constraints, auditing, final accountability). Quality / Verdict / Answerability triad — none automatable. Conceptual backing for the `governance_need: hard` scoring axis. | Ecosystem |
| **[cactus-compute/needle ⭐3.1k](https://github.com/cactus-compute/needle)** | 26M-parameter function-call model targeting phones, watches, and glasses. Simple Attention Network (GQA + RoPE, no FFN in encoder) beats FunctionGemma-270m, Qwen-0.6B, and Granite-350m on single-shot function-call benchmarks. Introduces "micro-device" as a new potential hardware tier below current edge profiles. | L1 |
| **[kangarooking/cangjie-skill ⭐2.9k](https://github.com/kangarooking/cangjie-skill)** | Seven-stage RIA-TV++ pipeline that distills books, video transcripts, and podcasts into deployable `SKILL.md` files for Claude Code / Cursor. Outputs `INDEX.md` cross-skill map, `DIGEST.md`, and adversarial test prompts. First tracked tool explicitly positioned as a production content-to-skill distillation pipeline. | L4b |
| **[juggler-ai/juggler ⭐146](https://github.com/juggler-ai/juggler)** | Native desktop GUI coding agent (Go + Wails, no Electron) organizing sessions as editable trees with Finder-style Miller columns. Tool calls are first-class visible UI objects. Multi-provider support; Yjs sync for multi-observer sessions. From Jules Storer (creator of the JUCE C++ audio framework). HN 103pts day-one. First "tree-structured session audit UI" signal. | L6 |
| **["Understanding is the New Bottleneck" — Geoffrey Litt](https://www.geoffreylitt.com/2026/07/02/understanding-is-the-new-bottleneck.html)** | MIT CSAIL researcher argues comprehension — not verification — is the binding constraint in multi-loop agentic work. Three techniques: literate diffs with embedded comprehension quizzes (speed regulators), micro-worlds for active participation, shared spaces for team mental models. Third signal today in a cluster supporting `comprehension_surface` as a new scoring axis. | Ecosystem |
| **[mattpocock/skills ⭐165k](https://github.com/mattpocock/skills)** | "Skills for Real Engineers" — Matt Pocock's personal `.claude/` directory. Two-signal rule met with `addyosmani/agent-skills`: "engineer-authored cross-agent skill pack" confirmed as canonical L4b sub-type. Highest-starred single-author skill pack tracked. | L4b |
...
Full analysis in [`docs/research-watch/`](docs/research-watch/) (454 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
