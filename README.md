...
## 🔥 What's hot right now (2026-07-17)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[lobehub/lobe-chat ⭐80.4k](https://github.com/lobehub/lobe-chat)** | "Chief Agent Operator" platform: hires, schedules, and reports on AI agent teams in 7×24 persistent operations. Four-pillar architecture — Operator, Create (10,000+ MCP Skills), Collaborate (Groups + Scheduling), Evolve (White-Box Memory). Mature: 2,850 releases, v2.2.10 July 10. Largest open-source user base in the multi-agent orchestration tier. First dedicated research-watch entry. | L2 |
| **[sourcebot-dev/sourcebot ⭐3.6k](https://github.com/sourcebot-dev/sourcebot)** | Self-hosted code intelligence platform explicitly designed for both humans and AI agents: citation-grounded NL Q&A about codebases, cross-repo regex search, IDE-level goto-definition across all repos. v5.1.2 released July 16. Self-hosted via Docker Compose — `data_sensitivity: confidential` compatible. First tracked L5 tool specialized for code-structure memory rather than conversation-fact memory. | L5 |
| **[LM Studio Bionic](https://lmstudio.ai/)** | LM Studio pivots from local model runner to a dedicated agent product ("An Agent made for Open Models") in initial preview. Local-first by design, ships day-one with JavaScript SDK, Python SDK, and CLI. Targets the `offline` + `confidential` segment occupied by Goose, Aider, and Continue — with LM Studio's large existing installed base as the adoption lever. | L1 |
| **Traceforce (YC S26)** | YC S26 company building security monitoring specifically for AI applications — runtime behavioral observability for agentic deployments. Monitors inter-agent traffic, input/output patterns, and policy violations in production. Second AI security signal on same day as ReasonGate: YC backing signals the AI app security monitoring market is becoming a standalone funded category. | L5 |
| **ReasonGate** | Python tool blocking LLM prompt injection attacks with an auditable human-readable reason for every block decision — "explainable security gate." Differentiates from threshold-based filters by making every block traceable and debuggable. Pairs with Traceforce: together they represent pre-inference defense (L3) + runtime monitoring (L5) as distinct specializations within the AI security stack. | L3 |
| **[Inkling — Thinking Machines Lab](https://thinkingmachines.ai/news/introducing-inkling/)** | 975B-parameter open-weights MoE (41B active) trained on 45T tokens of text, image, audio, and video. 1M-token context, controllable thinking effort, 30M+ RL rollouts. Open weights viable for `data_sensitivity: confidential` profiles that cannot use closed APIs. Highest HN-points signal 2026-07-16 (583 pts). | L1 LLM |
| **[xai-org/grok-build](https://github.com/xai-org/grok-build)** | xAI open-sources Grok Build: Rust-based full-screen TUI coding agent with headless CI/CD mode and ACP (Agent Client Protocol) support — second tracked L1 runtime to implement ACP natively alongside OpenInterpreter Rust. Added to registry. | L1 |
| **[openinterpreter/openinterpreter ⭐65.3k](https://github.com/openinterpreter/openinterpreter)** | Complete Rust rewrite with named harness emulation modes (`claude-code`, `deepseek-tui`, `qwen-code`) allowing behavioral switching against any backend. Native sandboxing, ACP editor integration. First tracked runtime with `harness_mode` as a runtime configuration primitive. | L1 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (472 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
