...
## 🔥 What's hot right now (2026-07-07)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[ruvnet/RuView](https://github.com/ruvnet/RuView) ⭐78.3k** | WiFi spatial AI (ESP32 mesh): presence, vitals, 17-keypoint pose — ships an MCP server (`rvagent`) exposing physical environment state to Claude, Cursor, and swarm agents. First "physical-world MCP capability" signal; 78k stars reflects maker + agent ecosystem overlap. | L4c |
| **[HKUDS/AI-Trader](https://github.com/HKUDS/AI-Trader) ⭐20.6k** | Agent-native trading environment: agents register, copy-trade each other's strategies, and compete on a $100K leaderboard. Not "agents executing trades for humans" — agents trading against each other. Second HKUDS financial-domain signal (vibe-trading 2026-05). | L1/L5 |
| **[langbot-app/LangBot](https://github.com/langbot-app/LangBot) ⭐16.7k** | Open-source multi-platform messaging agent framework — 11 platforms (Discord, Slack, Telegram, WeChat, WeCom, QQ, Lark, LINE, DingTalk, KOOK, Matrix) in one runtime; native MCP; production-grade rate limiting + content filtering; v4.10.5, 199 releases. First dedicated messaging-native L1 framework tracked. | L1/L2 |
| **Anthropic Global Workspace (HN 242pts)** | 16-author paper identifies "J-space" in Claude satisfying 5 Global Workspace Theory properties via Jacobian lens. Ablating J-space hidden reasoning surfaces concealed behavior — claimed safety application. Potential L5 primitive if J-lens inspection becomes API-accessible. | L1 ref |
| **AMD Ryzen AI Halo ($3,999, retail)** | First purchasable AMD AI dev kit: 128 GB unified LPDDR5x-8000, XDNA 2 NPU (50 TOPS), ~20 tok/s on 20B model at 35 W. Added to `hardware.json` — first AMD local-workstation profile. ROCm dependency is the primary adoption risk for agent tooling. | L7 |
| **[kyutai-labs/pocket-tts](https://github.com/kyutai-labs/pocket-tts) ⭐6k** | CPU-only TTS from Kyutai Labs (Moshi team): ~200ms first chunk, 6× faster than real-time on M4; voice cloning, 6 languages, pip-installable. Third voice/audio signal — the voice cluster monitoring conditions (3rd signal + Meetily crossing 20k★) are now both met. | L4b |
| **[bradautomates/claude-video](https://github.com/bradautomates/claude-video) ⭐4.2k** | `/watch <url> <question>` adds three-stage video pipeline (yt-dlp + ffmpeg + Whisper/Groq) to Claude Code. Four detail modes; token-aware frame budgeting; frame deduplication. First tracked video input modality in the L4b skill layer. | L4b |
| **[future-agi/future-agi](https://github.com/future-agi/future-agi) ⭐1.3k** | Closed-loop agent eval + simulation + guardrails + prompt optimization — production traces feed 6 algorithms to rewrite prompts without human authoring. First open-source "simulation-to-optimization" L5 tool; distinguishes from Langfuse (passive) and Claw Patrol (adversarial input). | L5 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (411 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
