...
## 🔥 What's hot right now (2026-07-20)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[PrefectHQ/fastmcp ⭐26,500](https://github.com/PrefectHQ/fastmcp)** | The dominant Python MCP framework: ~70% of all MCP servers, ~1M downloads/day, v1.0 absorbed into the official MCP Python SDK. v3.4.x adds SSRF/OAuth hardening, `fastmcp-remote` bridge (stdio→HTTP), `fastmcp-slim` client variant, and Prefect Horizon enterprise gateway (SSO, RBAC, audit logs). Also introduces "Apps" — interactive UI widgets rendered inside model conversations via MCP, a nascent L6 pattern. | L4 |
| **[langchain-ai/open-swe ⭐10,400](https://github.com/langchain-ai/open-swe)** | Open-source reference implementation of the background-async SWE agent pattern used at Stripe, Ramp, and Coinbase. Tasks dispatched to isolated cloud sandboxes (Modal/Daytona/E2B/Runloop), run unsupervised, surface as draft PRs. Multi-channel invocation (Slack, Linear, GitHub PR). Built on LangGraph + Deep Agents. Corridor guardrails via MCP. **Two-signal "background-async SWE harness" pattern confirmed** with background-agents (2026-07-13). | L2 |
| **[langchain-ai/openwiki ⭐12,600](https://github.com/langchain-ai/openwiki)** | 15-day-old CLI that auto-generates structured agent-consumable codebase wikis in Open Knowledge Format (OKF). CI/CD native (GitHub Actions, GitLab CI). Multi-provider LLM support. 12.6k stars in 15 days signals strong latent demand. **Two-signal "agent-optimized documentation layer" pattern confirmed** with Agent Docs for Markdown (2026-07-19). v0.2.0 July 16; pre-1.0 release pace is a risk for CI/CD use. | L5 |
| **[AstrBotDevs/AstrBot ⭐37,000](https://github.com/AstrBotDevs/AstrBot)** | IM-channel AI agent framework integrating 14+ platforms (WeChat, QQ, Telegram, Discord, Slack). LLM-agnostic backend (OpenAI, Anthropic, Ollama). 36k stars; Chinese-origin project with significant domestic adoption. Fills L2/L6 gap: IM-channel harness where end users interact through existing group chat apps rather than CLI/IDE. Schema gap: `channel_type: [cli | ide | web | im | voice]`. | L2/L6 |
| **[tirth8205/code-review-graph ⭐22,800](https://github.com/tirth8205/code-review-graph)** | Local-first code intelligence graph (#1 GitHub Trending all languages today, +1,876). MCP + CLI. 30 MCP tools, persistent SQLite codebase map, 82x token reduction claim, GitHub Action merge gates. L4c code-intelligence cluster leader. Two-signal watch active — independent signal pending for canonical L4c promotion. | L4c |
| **[kvcache-ai/ktransformers ⭐18,700](https://github.com/kvcache-ai/ktransformers)** | CPU-GPU heterogeneous MoE inference: AMX/AVX-optimized CPU kernels for cold experts + CUDA for hot experts. 4.62–19.74× prefilling speedup (ACM SIGOPS 2026 peer-reviewed). Targets DeepSeek-V3/R1 and MiniMax-M3 on consumer hardware. Apache 2.0. Two-signal consumer local large-model inference pattern confirmed with airllm. | L7 |
| **Kimi K3 (MoonshotAI)** | 2.8T MoE, 1M token context, $3/$15 per 1M. Self-reported FrontierSWE 81.2%. Weights expected 2026-07-27. Moonshot AI suspended subscriptions due to demand surge. | L1 hold |
| **[lobehub/lobe-chat ⭐80,400](https://github.com/lobehub/lobe-chat)** | "Chief Agent Operator" platform: agent team scheduling, 10,000+ MCP Skills, White-Box Memory. Largest open-source multi-agent orchestration platform. v2.2.10, 2,850+ releases. | L2 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (491 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
