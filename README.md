...
## 🔥 What's hot right now (2026-07-03)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **Apple Safari MCP Server** (webkit.org, HN 220 pts) | Official Apple/WebKit MCP server for browser-agent integration: 18 tools (navigation, page analysis, network, user interaction, debugging). Runs fully locally — zero network telemetry. Explicit Claude + Codex support. Together with Chrome DevTools MCP, **confirms "browser-vendor official MCP" as a named L4c sub-type** — two first-party browser vendors (Google + Apple) shipped official MCP capability layers within 72 hours. | L4c |
| **[ChromeDevTools/chrome-devtools-mcp](https://github.com/ChromeDevTools/chrome-devtools-mcp) ⭐45.1k** | Official Google Chrome DevTools MCP server: 40+ tools for performance tracing, network inspection, DOM automation, stack traces, memory debugging. Now confirmed as the first anchor of the browser-vendor official MCP sub-type. | L4c |
| **[safishamsi/graphify](https://github.com/safishamsi/graphify) ⭐76.9k** | `/graphify .` slash-command skill converting any codebase into a queryable knowledge graph. Tree-sitter AST extraction (36 languages, fully local) + LLM-based analysis for docs/PDFs/images/videos. 90-day star trajectory (viral after Karpathy LLM Knowledge Bases post). 151 releases; v0.9.5 July 2, 2026. | L4b |
| **[microsoft/agent-governance-toolkit](https://github.com/microsoft/agent-governance-toolkit) ⭐4.6k** | Kernel-level policy enforcement middleware for production AI agents. YAML/OPA/Cedar rulesets, SPIFFE/DID/mTLS zero-trust identity, merkle-based tamper-evident audit log. "Actions the kernel denies are structurally impossible." Adapters for LangChain, AutoGen, CrewAI, 7+ others. v4.1.0 June 9, 2026. First signal for L5 application-layer governance sub-type. | L5 |
| **[openai/codex-plugin-cc](https://github.com/openai/codex-plugin-cc) ⭐22.5k** | Official OpenAI plugin bridging Claude Code ↔ Codex CLI. Seven slash commands: code review, adversarial review, task delegation, session transfer. First cross-vendor agent delegation plugin — signals OpenAI treats Codex and Claude Code as complementary tools in one workflow. | L4c |
| **[Senior SWE-Bench](https://senior-swe-bench.snorkel.ai/)** | Princeton + UW–Madison benchmark for senior-level coding tasks: 31% shorter specs, 11-file avg span, hundreds of steps, "tasteful solve" scoring. Top scores: Claude Opus 4.8 (24%), Sonnet 5 (19.4%), GPT-5.5 (16%). Large spread → meaningful differentiator for clawfit `code-gen` scoring. | L5 |
| **[strands-agents/harness-sdk](https://github.com/strands-agents/harness-sdk) ⭐6.4k** | Production agent harness in Python + TypeScript. Five provider backends (Bedrock, Anthropic, OpenAI, Gemini, Ollama), MCP first-class, built-in observability, guardrails, self-correction. 68 releases; Bedrock support implies a `managed_inference` hardware posture not yet in clawfit's registry. | L2 |
| **[Manufact MCP Cloud](https://manufact.com) (YC S25)** | Purpose-built cloud deployment for MCP servers. Auto-deploy on `git push`, per-PR live previews, Cloud Inspector, cross-model test harness (GPT/Claude/Gemini). `mcp-use` SDK: 7M+ downloads. First MCP-specific hosting platform — schema gap: `mcp_hosting` field. | L7 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (375 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
