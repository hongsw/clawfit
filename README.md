...
## 🔥 What's hot right now (2026-07-24)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[Claude Opus 5](https://www.anthropic.com/news/claude-opus-5)** | Anthropic flagship released today (446 HN pts). Input price drops from $15/M (Opus 4) to **$5/M** — a 67% cut — while claiming 3× ARC-AGI 3 improvement and state-of-the-art SWE performance. Fast mode adds 2.5× speed at 2× price. Added to llms.json as `claude-opus-5`. | LLM |
| **[humanlayer/acp](https://github.com/humanlayer/acp)** | Kubernetes-native agent scheduler: agents, tools, tasks, and tool-calls as CRDs. Durable execution with checkpoints at each tool call; human approval gates and expert-contact channels are first-class primitives, not afterthoughts. MCP tool integration. OpenTelemetry built in. 146 HN pts today. | L2/L5 |
| **[citrolabs/ego-lite ⭐1,612](https://github.com/citrolabs/ego-lite)** | Inverts the dominant pattern of per-agent isolated headless browsers. Agents share the human's live macOS browser (authenticated sessions, cookies, bookmarks) in isolated Spaces. JS-function API reduces tool-call round-trips. Claimed 2.5× speed improvement. Works with Claude Code, Codex, Cursor. | L4c/L6 |
| **[izeigerman/claude-thermos](https://github.com/izeigerman/claude-thermos)** | Transparent reverse proxy that keeps the Claude prompt cache alive across idle windows in multi-agent sessions. Detects when the main agent is idle while a subagent runs, sends minimal warmup requests before the 5-minute TTL expires. Claimed ~20% API cost reduction. Zero code changes required. | L2 |
| **[open-multi-agent/open-multi-agent ⭐6,700](https://github.com/open-multi-agent/open-multi-agent)** | TypeScript multi-agent framework where a coordinator generates the task DAG from a goal at runtime — no graph authoring required. Checkpoint-based recovery, token/cost budgets, loop detection, stable run identity. Multi-model (Claude, GPT, Gemini, DeepSeek). MIT. | L2 |
| **[oraios/serena ⭐26,793](https://github.com/oraios/serena)** | Most cross-referenced MCP toolkit in the scan corpus. Exposes semantic code search, go-to-definition, find-references, and read/write editing as MCP tool calls — IDE-grade code intelligence for any agent. 16 months of sustained development. | L4c |
| **[diegosouzapw/OmniRoute ⭐28,487](https://github.com/diegosouzapw/OmniRoute)** | 290+ AI providers, 500+ models, 11 token compression engines in one MIT-licensed gateway. Grew from 8,500 to 28,487 stars in 24 days. Also trending today on GitHub. | L7 |
| **[mediar-ai/screenpipe ⭐20,400](https://github.com/mediar-ai/screenpipe)** | YC S26. Records screen and audio 24/7, stores queryable timestamped log locally, exposes to agents via MCP. First **passive ambient context** sub-type in corpus: agents query what you were working on without user annotation. | L5 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (512 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
