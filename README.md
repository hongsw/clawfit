...
## 🔥 What's hot right now (2026-07-02)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[openai/codex-plugin-cc](https://github.com/openai/codex-plugin-cc) ⭐22.5k** | Official OpenAI plugin bridging Claude Code ↔ Codex CLI. Seven slash commands: code review, adversarial review, task delegation, session transfer. First cross-vendor agent delegation plugin — signals OpenAI treats Codex and Claude Code as complementary tools in one workflow. Schema gap: `agent_delegation_target` for multi-runtime configurations. | L4c |
| **[browser-use/video-use](https://github.com/browser-use/video-use) ⭐13.7k** | Agent-native video editing skill: text-first architecture (agent reads transcripts + visual composites, never raw frames). Filler-word removal, silence cutting, color grading, subtitle burning, self-evaluation render loop. Fourth `content-creation` signal — schema addition warranted. Previously held at 8–9k★; +5k in 48h. | L4b |
| **[strands-agents/harness-sdk](https://github.com/strands-agents/harness-sdk) ⭐6.4k** | Production agent harness in Python + TypeScript. Five provider backends (Bedrock, Anthropic, OpenAI, Gemini, Ollama), MCP first-class, built-in observability, guardrails, and self-correction. 68 releases. Dual-language cadence is uncommon in this space; Bedrock support implies a `managed_inference` hardware posture not yet in clawfit's registry. | L2 |
| **[Senior SWE-Bench](https://senior-swe-bench.snorkel.ai/)** | Princeton + UW–Madison benchmark for senior-level coding tasks: 31% shorter task specs, 11-file avg span, hundreds of agent steps, "tasteful solve" scoring (correctness + bloat limits + codebase convention). Top scores: Claude Opus 4.8 (24%), Sonnet 5 (19.4%), GPT-5.5 (16%). Large spread → meaningful differentiator for clawfit `code-gen` scoring. | L5 |
| **[Manufact MCP Cloud](https://manufact.com) (YC S25)** | Purpose-built cloud deployment for MCP servers. Auto-deploy on `git push`, per-PR live previews, Cloud Inspector for in-browser MCP debugging, cross-model test harness (GPT/Claude/Gemini). SDK (`mcp-use`): 7M+ downloads, 10k+★. Named users: NASA, LangChain. First MCP-specific hosting platform — schema gap: `mcp_hosting` field for agent runtimes. | L7 |
| **[NVIDIA/skills](https://github.com/NVIDIA/skills) ⭐2.2k** | Official NVIDIA agent skills with OMS cryptographic signatures (verifiable against NVIDIA trust anchor). Covers CUDA-X, Jetson BSP (24 skills, full embedded device lifecycle), TAO Toolkit (57+ skills), Medical AI (DICOM/CT/MR/CXR), Physical AI. First hardware-vendor skill pack with verifiable provenance chain. Schema gap: `skill_provenance` + `embedded` hardware category. | L4b |
| **[micro/go-micro](https://github.com/micro/micro) ⭐23k** | First Go-native harness with dual-protocol coverage (MCP auto-generation + A2A). Composable agent interfaces. Anthropic + OpenAI co-sponsored. Breaks the Python-dominant harness pattern. | L2 |
| **[TencentCloud/CubeSandbox](https://github.com/TencentCloud/CubeSandbox) ⭐6.7k** | KVM microVM sandbox for agent code execution (RustVMM, <60ms boot, E2B-compatible, credential vault, egress audit). First cloud-provider-backed hardware-isolation service. Schema gap: `execution_isolation`. | L7 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (370 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
