...
## 🔥 What's hot right now (2026-07-29)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **AI Worms Through Copilot/Word (HN 249 pts)** | First documented self-propagating document-borne AI worm in production enterprise software (coordinated disclosure). White-on-white hidden instructions propagate through normal document-sharing workflows with no further attacker involvement. Structural finding: agents cannot separate "data to inspect" from "instructions to follow" — affects all L2/L4 agents reading third-party content. | Cross-layer security |
| **HANDBOOK.md Benchmark (HN 208 pts, arxiv 2607.25398)** | 65 agentic tasks across finance/medical/insurance/logistics/HR measuring agent policy-document adherence during multi-step tool use. Best model: 36.2% strict success. Four failure modes documented including "false compliance claims." First quantified evidence that the common enterprise governance pattern — "give the agent a long policy document" — is empirically unreliable. | L5 eval |
| **[drumih/turbo-fieldfare ⭐571](https://github.com/drumih/turbo-fieldfare) (HN 326 pts)** | Gemma 4 26B inference in ~2 GB RAM on any M-series Mac by streaming only active MoE experts from SSD. 5.1–6.3 tok/s on M2, 31–35 tok/s on M5 Pro. OpenAI-compatible HTTP server with tool function calling. First "SSD expert-streaming inference" signal — breaks the minimum-RAM assumption for consumer Apple Silicon. | L7/L1 inference |
| **[elder-plinius/T3MP3ST ⭐3.9k](https://github.com/elder-plinius/T3MP3ST)** | Autonomous red-team meta-harness wrapping existing coding agents (Claude Code, Codex, Hermes). No new API keys required — your resident agent is the brain. 90.1% pass@1 on XBEN (104-challenge suite); 8/10 real 2026 CVEs pinned to exact file/line/CWE. Claims verified with `npm run verify-claims`. Third signal for "domain-specialized meta-harness" pattern (security joins video + finance). | L2 security harness |
| **MCP 2026-07-28 Final Spec ratified** | RC tracked 2026-07-05 is now the official specification. New: `subscriptions/listen` (long-lived POST stream), OTel trace context, deterministic `tools/list` ordering for LLM cache hits. MRTR replaces all server-initiated requests. OAuth 2.0 DCR deprecated. L4c audit now mandatory for session-continuity tools. | Cross-layer protocol |
| **[synthetic-sciences/openscience ⭐1.45k](https://github.com/synthetic-sciences/openscience)** | Open-source AI workbench running the full scientific research loop (literature review → hypothesis → code → experiment → write-up) as a continuous agentic session. Domain specialist sub-agents (biology, physics, ML), critique sub-agent, MCP-native tool layer, local-first, model-agnostic. Open alternative to Anthropic Claude Science. First signal for open-source scientific research orchestration. | L3 research |
| **[HKUDS/OpenSpace ⭐7.1k](https://github.com/HKUDS/OpenSpace)** | Self-improving skill management system: retrieves task-appropriate skills dynamically, tracks outcomes, evolves skills via FIX/DERIVED/CAPTURED operations. 866 forks relative to 7.1k stars (12% fork ratio) — high customization rate. First signal for "self-improving skill management" as distinct from static skill packs. | L4b skill mgmt |
| **[vudovn/ag-kit ⭐7.95k](https://github.com/vudovn/ag-kit)** | TypeScript harness targeting Google Antigravity: 20 specialist agents, 47 reusable skills, 13 slash-command templates, persistent memory, native safety hooks, CI-backed validation. First harness signal explicitly targeting a Google-family LLM backend — exposes `primary_llm_target` schema gap. | L2 harness |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (541 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
