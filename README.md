...
## 🔥 What's hot right now (2026-08-05)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[cloudflare/computer ⭐2,370](https://github.com/cloudflare/computer) (GitHub Trending #1 all-languages, 2026-08-05)** | "Give your agent a computer." SQLite-backed virtual filesystem in a Durable Object with three pluggable execution backends (Container full Linux, Isolate Shell, Isolate JavaScript). Released August 3, 2026 as a preview. Architectural bet: persistent SQLite filesystem over container-per-agent eliminates cold-start penalties and state coordination across agent turns. Cap'n Web RPC sync daemon. MIT license. First "Durable Object agent filesystem substrate" signal. | L2 agent execution substrate |
| **[cloudflare/cloudflare-os ⭐1,400](https://github.com/cloudflare/cloudflare-os) (HN 317 pts, 2026-08-05)** | Cloudflare's internal enterprise agent workspace, open-sourced alongside cloudflare/computer. Agents access external systems only through Gatekeepers (Workers that mediate access and hold credentials — agents never hold credentials directly). Agents build sandboxed Gadget apps with disabled outbound network by default. Observation-tracking enforces downstream permissions on data derived from confidential sources. Most sophisticated data-lineage enforcement primitive in the corpus. MCP-compatible. | L2/L3 enterprise agent workspace |
| **[Mistral Shieldstral](https://mistral.ai/news/shieldstral/) (HN 287 pts, 2026-08-05)** | 3B open-weights multimodal safety classifier (Apache 2.0, Hugging Face). Policy-adaptive at inference time: supply plain-language safety rules as yes/no questions — no retraining required. Handles text + images from a single model. Matches or outperforms guard models up to 7× its size. Runs on a single 16GB GPU — fits alongside a local 7–13B agent model. First dedicated content-moderation LLM in the corpus; first signal for "policy-adaptive guard model" sub-type. | L3 open-weights safety filter |
| **[NovaSky-AI/SkyRL ⭐2,125](https://github.com/NovaSky-AI/SkyRL) (GitHub Trending Python, 2026-08-05)** | Modular full-stack RL library targeting long-horizon, tool-using LLM agents. Four components: skyrl-train (async training with in-flight weight updates), skyrl-agent (agent infrastructure), skyrl-gym (SWE-Bench, text-to-SQL, web search environments), skyrl-tx (Tinker API cross-platform implementation). Apache-2.0. First RL training framework explicitly targeting multi-turn tool use via gym environments — distinct from RLHF/preference tuning. | L1 RL training infrastructure |
| **[K-Dense-AI/scientific-agent-skills ⭐32,625](https://github.com/K-Dense-AI/scientific-agent-skills) (GitHub Trending Python, 2026-08-04)** | 158 ready-to-use skills across 18+ scientific domains (bioinformatics, drug discovery, proteomics, medical imaging, materials science, lab automation, data analysis, scientific communication) with 100+ scientific database integrations. MIT license; compatible with Claude Code, Codex, Cursor, Google Antigravity, Pi, Hermes, NemoClaw. Pairs cross-day with OpenScience (L3, July 29): two-layer scientific AI tooling stack now documented. | L4b scientific skill library |
| **[uber/ADR ⭐605](https://github.com/uber/ADR) (GitHub Trending Python #1, 2026-08-04)** | Uber's production security system for AI coding agents: ADR-Sensor (telemetry across 7+ tools on macOS/Linux/Windows), ADR-Bench (300+ adversarial tasks, 133 MCP servers, 17 attack techniques — largest open-source AI agent security benchmark), ADR-Detector (two-tier threat detection). MLSys 2026 accepted. Apache 2.0. First open-source defensive observability + benchmarking stack for enterprise coding agents. | L5/L2 enterprise agent security |
| **[huangruiteng/loopx ⭐1,523](https://github.com/huangruiteng/loopx) (GitHub Trending Python #4, 2026-08-04)** | Lightweight state kernel for long-running AI agent teams: persists objectives, typed todos with peer handoff verification, human-blocking gates, evidence logs, and quota tracking with auto-wake — across agent restarts and handoffs between different agent runtimes. Agent-agnostic (Claude Code, Codex, Cursor). Philosophy: "Keep the loop moving. Keep the judgment human." First "durable agent state kernel" signal — distinct from session-scoped harnesses. | L2/L3 durable state kernel |
| **Hoplite (hoplite.sh — HN Launch HN YC S26, 2026-08-04)** | YC S26-backed cloud service running AI coding agents in isolated per-task dev environments, verifying changes via tests and browser preview, then opening PRs for human review. First cloud coding agent deployment platform with mandatory human approval gate + MCP server + CLI access. Distinct from IDE extensions (Cursor) and bare CLIs (aider): fully managed cloud isolation with built-in CI verification. | L2 cloud coding agent |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (577 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
