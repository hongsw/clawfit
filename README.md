...
## 🔥 What's hot right now (2026-07-05)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[asheshgoplani/agent-deck](https://github.com/asheshgoplani/agent-deck)** | "Mission control" TUI for AI coding agents — one terminal for Claude Code, Gemini CLI, OpenCode, Codex. Session forking, MCP socket pooling, Conductor meta-sessions (persistent orchestrators that watch + auto-respond across other sessions). First terminal-native multi-agent session manager with a full L2 harness + L6 human-interface surface. Star count unverified (432 confirmed vs 46.4k claimed — provenance flagged). | L2/L6 |
| **[microsoft/intelligent-terminal](https://github.com/microsoft/intelligent-terminal)** | Windows Terminal fork with native ACP (Agent Client Protocol) integration — auto-detects Claude/Codex/Gemini/Copilot CLIs, injects shell context via stdio. ACP is now a **3-vendor cross-platform standard** (Zed origin, JetBrains Oct 2025, Microsoft Build 2026). Terminal-as-first-class-ACP-host is a distinct L7 pattern from IDE-as-ACP-host. | L7/L1 |
| **[crynta/terax-ai](https://github.com/crynta/terax-ai) ⭐8k** | 7 MB terminal-first AI dev workspace (Tauri 2 + Rust) — BYOK multi-provider including Ollama/MLX local models. Binary footprint is a documented design constraint (7.4 MB vs Warp's 423 MB). First L1 runtime in this series where size is the headline spec. Competes in `offline_mid_codegen` vs Goose and Aider. Critical OSC 8888 vuln fixed in latest release. | L1/L7 |
| **[MCP 2026-07-28 Stateless RC](https://blog.modelcontextprotocol.io/posts/2026-07-28-release-candidate/)** | Official MCP release candidate eliminates Mcp-Session-Id, the initialize handshake, and sticky routing. Protocol version + capabilities now travel in `_meta` on every request — stateless horizontal scaling becomes protocol-native. Every tracked MCP server requires migration. Effective July 28, 2026; beta SDKs available now. | L4c/L5 |
| **["Better Models: Worse Tools"](https://lucumr.pocoo.org/2026/7/4/better-models-worse-tools/) (HN 44pts)** | Armin Ronacher (Flask/Jinja creator) documents empirically that Opus 4.8 and Sonnet 5 produce malformed tool calls against strict schemas — but succeed in Claude Code's forgiving harness. Names the pattern: "forgiving harness overfitting." Flags `llm.tool_schema_compliance` and `agent.harness_schema_strictness` as candidate new scoring axes. | L2 meta |
| **[alirezarezvani/claude-skills](https://github.com/alirezarezvani/claude-skills) ⭐20k** | 337-skill cross-agent pack covering 10+ coding agents. Extends into non-engineering domains: marketing, compliance, C-level advisory. Third large community skill aggregator; first with genuine multi-agent portability as a design constraint. | L4b |
| **[Zackriya-Solutions/meetily](https://github.com/Zackriya-Solutions/meetily) ⭐14.9k** | Privacy-first fully local AI meeting assistant (Rust). 100% on-device: Parakeet/Whisper STT, SortFormer diarization, Ollama summarization. First `task: meeting-notes` vertical signal; pairs with huggingface/speech-to-speech as two-signal local voice/audio cluster. | L1/L6 |
| **[CoplayDev/unity-mcp](https://github.com/CoplayDev/unity-mcp) ⭐11.5k** | MCP server running inside the Unity Editor process — asset management, scene control, script editing. Extends "environment-native MCP" from browsers to game engines. Supports Claude, Cursor, Copilot, Gemini, OpenAI simultaneously. | L4c |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (387 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
