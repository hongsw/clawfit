...
## 🔥 What's hot right now (2026-07-26)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **Open-weight AI Is Having Its Kubernetes Moment** | 296 HN points, 239 comments. The Kubernetes arc: complex enterprise-only infrastructure → commoditized default substrate. If correct for open-weight AI, proprietary APIs shift from default to option — reshaping clawfit's `network` and `hardware` filter dimensions for mid-to-large org users. Macro signal, no specific tool. | cross-layer |
| **Context engineering rules for Claude 5 models** | Anthropic's post-Claude Opus 5 context engineering guidelines claim simplified system prompts achieve parity with exhaustive prompt structures. Counter-signal to "specsmaxxing" (2026-05-03). If verified across harnesses, L2 tools that embed elaborate CLAUDE.md structures may face diminishing returns. 121 HN pts + GeekNews front page. | L2 |
| **[jgravelle/jcodemunch-mcp ⭐2.2k](https://github.com/jgravelle/jcodemunch-mcp)** | AST-indexed symbol-level code retrieval MCP server via tree-sitter. Index once, query cheaply: symbol lookup, blast-radius analysis, dead-code detection as named MCP tool calls. Claims 95%+ token reduction on code exploration. Second signal for "code intelligence MCP server" sub-type (first: serena ⭐26.8k, 2026-07-23). Compatible with Claude Code, Cursor, Windsurf, Continue. | L4c |
| **[sktkkoo/yorishiro](https://github.com/sktkkoo/yorishiro)** | macOS terminal where AI agents are first-class residents, not assistants. Design inverts standard AI-terminal integration — agents are the primary user, humans observe. Distinct from coding-agent TUIs (Grok Build, Terax); targets an agent-native *host* environment. First signal for "agent-native terminal substrate." | L1 |
| **[can1357/oh-my-pi ⭐19.7k](https://github.com/can1357/oh-my-pi)** | Terminal coding agent with embedded LSP + DAP + MCP as first-class tool primitives — not subprocess wrappers. Native Rust engine (~55k lines), 32 built-in tools, 40+ LLM backends, content-hash-anchored edits. ~400 releases since Dec 2025. Second signal; first full L2 production agent assessment. | L2 |
| **[MODSetter/SurfSense ⭐15.5k](https://github.com/MODSetter/SurfSense)** | Open-source NotebookLM alternative exposing 10+ live social + web scrapers (Reddit, YouTube, TikTok, Google Maps, Amazon, etc.) simultaneously as REST endpoints and native MCP tools. Agents call `surf_reddit` or `surf_web` as MCP tool calls — no user copy-paste. First "live-data-connector MCP server" signal. | L4c/L5 |
| **Claude Opus 5 (Anthropic)** | Frontier reasoning model ($15/$75 per 1M tokens, 200k context) in `llms.json`. SWE-bench Verified 72.5%, GPQA Diamond 78.0%. Current top scorer in clawfit registry for `task: research` and `task: code-gen` profiles. | L1 LLM |
| **UK AISI/Caisi Kimi K3 Cyber Assessment** | First government safety institute evaluation (UK AISI + Caisi, NIST-hosted) of a production frontier coding model's offensive cybersecurity capabilities. Kimi K3 (FrontierSWE 81.2%) assessed. Signals need for `safety_assessments` field in governance-hard profiles. 114 HN pts. | L1 gov. |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (522 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
