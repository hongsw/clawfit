...
## 🔥 What's hot right now (2026-07-27)
...
| **Microsoft MAI-Cyber-1-Flash + MDASH** | Security-specialized LLM inside MDASH (multi-agent vulnerability identification harness). Released 2026-07-27; 95.95% CyberGym, claims 50% cost vs. leading models. Demonstrates production domain-specialized model + cost-tiered escalation routing in enterprise multi-agent deployment. No public repo. | L1/L2 |
| **[AlmanacCode/codealmanac ⭐703](https://github.com/AlmanacCode/codealmanac)** | Locally-maintained markdown wiki for AI coding agents storing architectural decisions, invariants, and workflow constraints that code alone can't carry. Three lifecycle agents (build/ingest/garden). Second signal for L5 "curated codebase context" alongside openwiki (2026-07-20). Schema gap: `memory_content_model`. | L5 |
| **[RyanAlberts/best-of-Agent-Harnesses ⭐404](https://github.com/RyanAlberts/best-of-Agent-Harnesses)** | 140+ harnesses catalogued with MCP server (`recommend`, `pick_harness`, `compare` tool calls), `harnesses.json`, and `llms.txt`. An agent can query harness recommendations via MCP. First "agent-queryable harness catalog" signal — directly relevant to clawfit's potential MCP surface. | L4 |
| **Netflix In-House LLM Serving** | Engineering blog: Netflix integrates vLLM + Triton into existing ML infrastructure. Documents vLLM selection rationale over TensorRT-LLM, two Triton backend modes, Red-Black vs. Versioned deployment. Hyperscaler convergence on open-source inference stack. L7 reference architecture. | L7 |
| **[dmsdc-ai/telepty](https://github.com/dmsdc-ai) (Show GN)** | Lightweight control plane for AI CLI sessions (Claude Code, etc.) across multiple developer machines. "Multi-machine session control" framing distinct from single-machine harnesses (oh-my-pi, Hermes) and full multi-agent platforms. First signal for this architectural gap. Schema gap: `deployment_scope`. | L2–L3 |
| **[experientiallabs/world-model-optimizer](https://github.com/experientiallabs/world-model-optimizer) (HN 20 pts)** | Claims frontier-quality inference from small models at half cost via distillation. If verified for agent-relevant tasks, challenges cost assumptions in clawfit's LLM recommendation dimension. Early signal; org unknown. Schema gap: `distillation_available`. | L1 |
...
Full analysis in [`docs/research-watch/`](docs/research-watch/) (529 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
