...
## 🔥 What's hot right now (2026-07-04)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[alirezarezvani/claude-skills](https://github.com/alirezarezvani/claude-skills) ⭐20k** | 337-skill cross-agent pack covering 10+ coding agents (Claude Code, Codex, Gemini CLI, Cursor, and 8 others). Extends into non-engineering domains: marketing, compliance, C-level advisory, finance. Third large community skill aggregator tracked; first one with genuine multi-agent portability as a design constraint. | L4b |
| **[Zackriya-Solutions/meetily](https://github.com/Zackriya-Solutions/meetily) ⭐14.9k** | Privacy-first, fully local AI meeting assistant (Rust, macOS + Windows). Parakeet/Whisper transcription, SortFormer speaker diarization, Ollama summarization — 100% on-device, no cloud. First signal for `task: meeting-notes` as a clawfit vertical; local voice/audio AI cluster forming. | L1/L6 |
| **[CoplayDev/unity-mcp](https://github.com/CoplayDev/unity-mcp) ⭐11.5k** | MCP server running inside the Unity Editor process (not an external proxy): asset management, scene control, script editing, task automation. Extends the "environment-native MCP" pattern from browsers to game engines. Active today; explicit support for Claude, Cursor, Copilot, Gemini, OpenAI. | L4c |
| **[huggingface/speech-to-speech](https://github.com/huggingface/speech-to-speech) ⭐5.3k** | HuggingFace-maintained framework for fully local voice agents: STT (Whisper/Faster Whisper) → LLM → TTS (Parler-TTS/MeloTTS/Kokoro). Modular model swap, Apache 2.0, no API key. First Tier-1 ML org voice pipeline tracked. Pairs with Meetily as a two-signal cluster for local voice/audio AI. | L4b |
| **facebook/astryx ⭐4.6k** | Meta's agent-ready design system (150+ React/StyleX components) shipping an MCP server + CLI as first-class primitives. API surface is identical for human developers and AI agents by design. Validated across 13,000+ Meta apps (Facebook, Instagram, WhatsApp, Threads). First tracked "agent-native component library." | L4b |
| **[safishamsi/graphify](https://github.com/safishamsi/graphify) ⭐76.9k** | `/graphify .` slash-command skill converting any codebase into a queryable knowledge graph. Tree-sitter AST (36 languages) + LLM-based analysis for docs/PDFs/images/videos. 90-day viral trajectory; 151 releases; v0.9.5 July 2, 2026. | L4b |
| **[jamesob/local-llm](https://github.com/jamesob/local-llm) ⭐666** | "Everything I know about running LLMs locally" — practitioner guide created July 3, 2026. 388 HN pts in 24 hours (expert audience, inverted star-to-HN ratio). Relevant to `hardware: local` and `network: offline` scoring accuracy; endorsed tools may see star spikes in the next 7–14 days. | L7 ref |
| **[kerlenton/mcpsnoop](https://github.com/kerlenton/mcpsnoop) ⭐127** | "Wireshark for MCP" — transparent proxy capturing live JSON-RPC between AI client and MCP server. Hung-call detection, tool-call replay, capability handshake inspector. First signal for MCP protocol debugger as a distinct L4c sub-type (separate from MCP Inspector). | L4c |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (382 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
