...
|--------|---------------|-------|
| **[planning-with-files](https://github.com/OthmanAdi/planning-with-files) ⭐24.6k** | Crash-proof file-based planning (task_plan.md + findings.md + progress.md) that survives context loss and /clear; works with 60+ agents via SKILL.md; deterministic completion gate; /plan-loop composes with Claude Code /loop. Manus-style persistent agent planning. | L5/L4b |
| **[free-llm-api-resources](https://github.com/cheahjs/free-llm-api-resources) ⭐25.4k +510/day** | Code-generated catalog of free-tier LLM API endpoints across 14+ providers with per-model rate limits. 25k stars for a reference list (exceeds Aider) — measures inference cost as top-of-funnel practitioner priority. LLM commoditization signal. | L7 |
| **[nano-vllm](https://github.com/GeeeekExplorer/nano-vllm) ⭐14.4k** | Minimal 1,200-line Python reimplementation of vLLM — implements prefix caching, tensor parallelism, CUDA graphs while omitting PagedAttention. 1,434 vs 1,362 tok/s vs full vLLM. nanoGPT tradition applied to inference. | L7 |
| **[OpenClaw-RL](https://github.com/Gen-Verse/OpenClaw-RL) ⭐5.4k** | Princeton/ICML-2026 async RL framework that trains LLMs from live conversations; decouples agent serving, rollout collection, reward eval, and policy training into independent async loops — none blocking the others. First signal for "continuously self-improving runtime." | L1 |
| **[pxpipe](https://github.com/teamchong/pxpipe) ⭐~2.3k** | Local TypeScript proxy that renders text context as dense PNGs before forwarding to Anthropic/OpenAI APIs, exploiting image-token price asymmetry (3.1:1 chars/token). Claims 59–70% cost reduction. Known fidelity loss on hex/UUIDs (~7% misread). | L7 |
| **MCP 2026-07-28 RC — stateless core** | Official spec RC removes session layer entirely (SEP-2567); any request can land on any server instance; three new required headers per call. Hard breaking change for all existing MCP servers using session continuity. Horizontal scaling now protocol-native. | L4c |
| **["Better Models: Worse Tools"](https://lucumr.pocoo.org/2026/7/4/better-models-worse-tools/)** | Armin Ronacher (Flask/Jinja author): Claude Opus 4.8/Sonnet 5 produce malformed tool calls (~20% rate) on non-Claude-Code schemas while Opus 4.5/Codex do not. Mechanism: permissive harness removes gradient signal for schema violations during post-training. Strict mode mitigates. | L2→L4c |
| **[terax-ai](https://github.com/crynta/terax-ai) ⭐8k** | Tauri 2/Rust terminal AI workspace: 7.4 MB binary (~60× smaller than Warp), <300 ms cold-start claim, full offline via Ollama/MLX. Approval-gated tool execution; TERAX.md auto-generated project memory. First footprint-constrained terminal-native workspace signal. | L1/L7 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (357 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
