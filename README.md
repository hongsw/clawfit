...
## 🔥 What's hot right now (2026-08-01)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[zhaoxuya520/reverse-skill ⭐11,681](https://github.com/zhaoxuya520/reverse-skill) (GitHub Trending #4)** | AI-powered skill router for authorized reverse engineering and penetration testing; 1,360 stars today. Supports Claude Code, Kiro, Cursor, Cline. Router architecture (not a static pack) with self-evolving KB claim. 15.2% fork ratio — highest in the security skill cluster. Fourth security-domain agent skills signal. | L4b security skill router |
...
| **WASTE — NVMe weight-streaming for Kimi K3 (HN 305 pts)** | Dependency-free C inference engine running 2.78T-param Kimi K3 on a 64 GB consumer machine by streaming MoE expert weights from NVMe. 0.62 tok/s. Second signal for the `inference_strategy: ssd-streamed` pattern (after turbo-fieldfare on Gemma 4 26B). Cross-day two-signal confirmation for this schema dimension. | L7 infrastructure |
| **Fermisense: $500 RL fine-tune beats frontier (GeekNews 35 pts)** | Qwen3.5-9B fine-tuned with GRPO for $500 GPU time outscores best frontier configs at catalog review: 87.3% vs 76.9%, with 68–340× lower per-inference cost ($0.50/1k vs $19–$172/1k). Public HuggingFace weights. First production case for task-specialized RL fine-tunes beating frontier APIs at agentic tasks. | L1 cost signal |
| **Manifest: Why we deprecated our LLM router (HN 82 pts)** | Post-mortem on 4 months of LLM routing for 7,000 users. Routing decisions made before tool calls are stale by execution time; cache locality savings (75–90%) outweigh routing gains; model consistency has hidden value for evals. Validates clawfit's no-router design. | Cross-layer anti-pattern |
| **DeepSeek V4 Flash 0731 (HN 568 pts)** | Agent-optimized MoE: Terminal Bench 82.7, DeepSWE 54.4, Toolathlon 70.3. $0.14/$0.28/M, $0.003/M cache hit (98% discount — lowest for frontier-class). 284B total / 13B active, MIT open weights, Responses API native. | L1 MoE LLM |
| **Frontier Lab Agent Intrusion Timeline (HN 266 pts)** | First public post-mortem of a real frontier-lab agent security incident. Four-signal cluster for "agent security boundary failure" confirmed: Prismata + AI Worms + HANDBOOK.md + this. Pattern promotion candidate. | Cross-layer security |
| **[trailofbits/skills ⭐6,364](https://github.com/trailofbits/skills)** | Trail of Bits' security-domain Claude Code skill pack: 7 categories. First institutional security firm releasing a signed Claude Code skill pack — distinct from community aggregators by provenance and maintenance accountability. | L4b security skills |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (555 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
