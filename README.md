...
## 🔥 What's hot right now (2026-08-07)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[AMD acquires Taalas](https://www.theregister.com/systems/2026/08/06/amd-acquires-ai-chip-startup-taalas-to-boost-inference-performance-by-etching-models-into-silicon/) (HN 826 pts, 2026-08-07)** | Taalas physically encodes model weights into chip circuitry (4-bit multiplier cells, top-metal routing), eliminating DRAM bandwidth bottleneck entirely. HC1 chip achieves ~15,000 tok/sec on 8B Llama at ~30mm². Trade-off: model locked at tape-out. AMD integrates into Instinct inference line. First "weights-in-silicon" hardware class in the corpus — a new inference sub-type below L1. | Hardware Inference Substrate — weights-in-silicon |
| **[PrimeIntellect/prime-agent ⭐6,000](https://github.com/PrimeIntellect-ai/prime-agent) (GitHub Trending #1 all-langs, 2,271 today, 2026-08-07)** | Self-improving RLM coding agent (TypeScript/Python, v0.7.0 Aug 5). Two structural primitives: RLM treats context as mutable variables; Continual Harness stores durable skill descriptions across sessions. `/refine` reviews agent trajectories and updates harness state without touching base system prompt — self-improvement loop from observed evidence. Daemon-backed, peer-to-peer multi-agent, ACP mode. Distinct from prime-rl (RL training infra). | L1/L2 self-improving RLM agent |
| **[Cloudflare Kitesurf](https://blog.cloudflare.com/kitesurf/) (HN 88 pts, 2026-08-07)** | Agent-first browser built on Cloudflare Workers (Rust/WASM: Blitz, Stylo CSS, Boa JS). CDP-compatible — Puppeteer/Playwright drop-in via `browser=kitesurf`. 3.1–3.8x less CPU, 4.7–7.0x less memory vs. Chromium for agent web tasks; 1.7–1.8x slower wall time (deliberate tradeoff). 215k+ WPT. Free beta. Open-source commitment. First purpose-built agent browser infrastructure in the corpus. | L4/L7 agent-first browser substrate |
| **[wshobson/agents ⭐38,600](https://github.com/wshobson/agents) (GitHub Trending Python, 2026-08-07)** | Multi-harness plugin marketplace: single Markdown source generates native artifacts for Claude Code, Codex CLI, Cursor, OpenCode, Gemini CLI, and GitHub Copilot simultaneously. 94 plugins / 203 agents / 175 skills / 109 commands / 16 orchestrators. `plugin-eval` quality framework (static + LLM-judge + Monte Carlo). Tiered model strategy (Fable 5 → Opus → Sonnet → Haiku) encoded in plugin metadata. First formal cross-harness adapter build system in the corpus. | L4 cross-harness plugin marketplace |
| **[semantica-agi/semantica ⭐2,255](https://github.com/semantica-agi/semantica) (GitHub Trending Python, 2026-08-07)** | "Open Source Palantir for AI Agents." Graph-native context infrastructure (RDF + LPG polyglot backends) with `record_decision()` provenance, `trace_decision_chain()` causal ancestry, W3C PROV-O export for regulatory compliance. Deterministic reasoning (forward chaining, Rete, Datalog, SPARQL) without LLM calls. First-class MCP server (15+ tools). v0.6.0 July 21, 2026. First L5 signal for compliance-grade decision provenance. | L5 graph-native context + compliance provenance |
| **[Discovery Loop](https://discoveryloop.com) (HN 536 pts, 2026-08-06)** | Jeff Dean, Sanjay Ghemawat, Quoc Le, and Oriol Vinyals departed Google to co-found a startup automating the scientific experimental loop (propose → execute → learn) at scale. Radical Ventures + Khosla Ventures; Google as founding investor. Highest-credential team in the corpus and the most direct L5 (automated research loop) embodiment to date. | L5 automated research loop |
| **[Atlassian Rovo prompt injection](https://www.promptarmor.com/resources/atlassian-rovo-exfiltrates-data) (HN 155 pts, 2026-08-06)** | Zero-click indirect prompt injection in Atlassian Rovo bypasses org-level web search controls, exfiltrating any data via Rovo's connectors (Jira, Confluence). No user action required. First documented real-world enterprise breach via indirect prompt injection in the corpus. | L3 enterprise agent security |
| **[google-gemini/gemini-cli ⭐106,400](https://github.com/google-gemini/gemini-cli) (GitHub Trending, 2026-08-06)** | Google's official open-source terminal agent: Gemini 3 models, 1M-token context, native Google Search grounding, MCP client, conversation checkpointing. Free tier: 1,000 req/day. Two-signal cluster with Meta Muse Code (hyperscaler-native terminal agents). | L1 Google-native terminal agent |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (590 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
