...
## 🔥 What's hot right now (2026-07-15)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[openinterpreter/openinterpreter ⭐65.3k](https://github.com/openinterpreter/openinterpreter)** | Complete Rust rewrite of Open Interpreter; design center: "coding agent for low-cost models." Named harness emulation modes (`claude-code`, `deepseek-tui`, `qwen-code`) allow runtime behavioral switching against any LLM backend. Native sandboxing (macOS/Linux/Windows), ACP editor integration, computer-use capabilities. First tracked runtime with `harness_mode` as a runtime configuration primitive. | L1 |
| **[HKUDS/nanobot ⭐45.7k](https://github.com/HKUDS/nanobot)** | Lightweight self-hosted AI agent with WebSocket gateway serving Telegram, Discord, Slack, WeChat, Feishu, Mattermost, and email from a single agent loop. Multi-provider LLM (Claude/ChatGPT/Ollama/vLLM); "Dream" persistent memory; native MCP tool support. v0.2.2, June 2026. Introduces `statefulness: persistent-cross-channel` as a new tier candidate. | L2 |
| **[Agnost AI (YC S26)](https://www.agnost.ai)** | Production monitoring platform that reads real agent conversations to detect failure categories (broken workflows, retries, setup friction, churn risk) then opens PRs with proposed fixes. OpenTelemetry-native; claimed 2-minute setup; early clients include Google and Exa. First "evaluation-to-fix loop" signal — moves eval output from dashboards to actionable PR artifacts. | L5 |
| **[PrismML Bonsai 27B](https://prismml.com)** | 1-bit quantized 27B-parameter model (3.9 GB) confirmed to run on iPhone; ternary variant (5.9 GB) targets laptop. Tool calling and multi-step reasoning on-device. 14× less memory, 8× faster than 16-bit. Expands clawfit's viable local execution tier to include phones for 27B-class capability. | L1 |
| **["Wrapping the Unpredictable Genius"](https://melodykoh.substack.com/p/wrapping-the-unpredictable-genius)** | Melody Koh articulates harness engineering as the real competitive moat: model = commodity; the deterministic "wrap" is the defensible asset. Four-layer model: model → harness → documentation (advisory) → hooks (enforcing, independent of model agreement). Maps directly to Claude Code's pre-tool-call hook mechanism. Second signal on harness-as-moat axis alongside 2026-07-14's "Own the Outer Loop." `hooks_enforcement` schema candidate. | Ecosystem |
| **["Software Ate the World, Now Hardware Is Eating Software"](https://wing.vc/content/software-ate-the-world-now-hardware-is-eating-software/)** | Wing VC quantifies AI application margin compression: 50-60% gross margins vs SaaS 75-90%, because inference costs average 23% of revenue and don't decline at scale. Generic AI wrappers trade at 5-8× revenue vs SaaS 6.7×. Provides economic rationale for clawfit `sustainability_tier` scoring: infrastructure-layer tools more durable than application-layer wrappers. | Ecosystem |
| **[PrimeIntellect-ai/prime-rl ⭐1.7k](https://github.com/PrimeIntellect-ai/prime-rl)** | Open-source agentic RL framework for trillion-parameter MoE training on 1,000+ GPUs with fully asynchronous rollout/optimization (concurrent, not synchronized). PyTorch FSDP2, FP8, vLLM inference for rollouts, Verifiers environments hub for software engineering tasks. v0.7.0, July 14, 2026. First tracked training infrastructure explicitly targeting agentic RL. `training_method` schema candidate for llms.json. | L7 |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (462 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
