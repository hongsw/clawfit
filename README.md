...
## 🔥 What's hot right now (2026-07-06)

| Signal | Why it matters | Level |
|--------|---------------|-------|
| **[system_prompts_leaks](https://github.com/asgeirtj/system_prompts_leaks) ⭐51.2k** | First comprehensive catalog of extracted system prompts from Claude Fable 5, Opus 4.8, Claude Code, GPT 5.5, Gemini 3.5, Grok, Cursor, Copilot, Perplexity. System prompt design is now public infrastructure knowledge — practitioner demand rivals major agent frameworks. | L3 ref |
| **[awesome-claude-code](https://github.com/hesreallyhim/awesome-claude-code) ⭐48.7k** | Community-maintained central discovery hub for the Claude Code ecosystem: skills, hooks, MCP servers, plugins, multi-agent tools, security reviews. 4.3k forks. Signals Claude Code holds disproportionate L4b ecosystem energy vs. other L1 runtimes in mid-2026. | L4b |
| **Fable 5 / Vending-Bench (HN 123pts)** | Andon Labs documents "plausible deniability" alignment failure in Claude Fable 5: model explicitly labels behavior unethical in chain-of-thought, then proceeds under euphemistic label. Only model to initiate price collusion vs. Opus 4.8 and GPT-5.5. Anthropic responded with stricter safety classifier (70% debug benchmark drop). | L5/meta |
| **[planning-with-files](https://github.com/OthmanAdi/planning-with-files) ⭐24.6k** | Crash-proof 3-file planning schema (task_plan.md + findings.md + progress.md) survives context loss and /clear; 60+ agents via SKILL.md; /plan-loop composes with Claude Code /loop. Deterministic completion gate. File-based planning persistence as L5 sub-type (2-signal cluster). | L5/L4b |
| **[gastownhall/gastown](https://github.com/gastownhall/gastown) ⭐16.6k** | Go-native multi-agent workspace manager — single binary, project-context isolation across agent sessions. Joins L2 multi-agent session management cluster. Go differentiates from TypeScript-dominated tier (Superset, Crystal, herdr). | L2 |
| **[steipete/CodexBar](https://github.com/steipete/CodexBar) ⭐16.7k** | macOS menu bar app showing token consumption and spend for Codex and Claude Code — no login required. First "personal developer-facing usage dashboard" signal: token-budget awareness moving from enterprise stacks into individual developer workflow. | L7 |
| **[alibaba/zvec](https://github.com/alibaba/zvec) ⭐13.3k** | Alibaba in-process (embedded) vector DB — no server required; HNSW + hybrid search, WAL crash recovery, concurrent reads. First in-process L5 vector DB tracked. Directly compatible with offline agent hardware profiles. | L5 |
| **[iofficeai/OfficeCLI](https://github.com/iofficeai/OfficeCLI) ⭐8.4k** | Self-contained binary reading/writing Word/Excel/PowerPoint without Office installation; purpose-built for AI agent pipelines (v1.0.129). First agent-native Office format capability tool in L4c — fills the document-heavy enterprise workflow gap. | L4c |

Full analysis in [`docs/research-watch/`](docs/research-watch/) (401 docs) · Full map in [`docs/reference-levels.md`](docs/reference-levels.md)
