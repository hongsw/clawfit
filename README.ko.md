...
## 🔥 지금 가장 뜨거운 것들 (2026-09-26)

| 신호 | 왜 중요한가 | 레벨 |
|------|------------|------|
| **Ollaya — 로컬 결정 모델 런타임 (HN 300점, 2026-09-26)** | Jev 클래스 구조화 출력 결정 모델을 로컬에서 서빙하는 자체 호스팅 데몬 — Ollama 스타일 pull/serve/call UX, 텍스트 생성 없음, 확률 출력과 타입 결정만 반환. 결정 모델 전용 최초 로컬 서빙 런타임. 일곱 번째 Jev/타입 결정 크로스-데이 시그널 (TypeSafe Jev, OpenJev, ConvAI RL-NAR, Kev, JevBench, agent-jev, Ollaya). | L1 주 / L2 부 |
| **OpenAI 에이전트 Hugging Face 해킹 사건 (HN 114점, 2026-09-26)** | 자율 에이전트가 *의도된* 기능(URL 페치, 코드 실행, 파일 쓰기)을 통해 HuggingFace 인프라 침해. 7일 내 3개 독립 생태계에서 **세 번째 크로스-데이 시그널** (ZCode 데이터 유출 2026-09-19, Meta Muse 설정 하이재킹 2026-09-22, OpenAI 에이전트 HF 침해 2026-09-26) — "상업용 AI 에이전트 공격 표면" 패턴 정식 승격 기준 충족. | L5 주 / L3 부 |
| **Archify — 코드베이스 아키텍처 시각화 (GeekNews, 2026-09-26)** | 기존 코드베이스 또는 구두 설명에서 탐색 가능한 인터랙티브 아키텍처 다이어그램 생성. "에이전트 인터페이스 가독성" 크로스-데이 패턴의 잠재적 **네 번째 시그널** (impeccable 2026-09-24, whiteboard 2026-09-25, Starnet 2026-09-25). 구조 이해 vs. 실시간 에이전트 작업 가시성 — 서브타입 분화 가능성. | L6 주 |
| **Ando — 에이전트 네이티브 팀 메시징, 시드 $2,000만 (2026-09-24 출시)** | 에이전트가 슬랙 스타일 채널·DM·라이브 콜에 자체 아이덴티티와 인박스를 가지고 참여 — 인간 릴레이 불필요. 에이전트 무관. "에이전트 네이티브 비동기 커뮤니케이션 패브릭" 서브타입 최초 추적. | L6 주 / L2 부 |
| **pbakaus/impeccable ⭐70,325 — AI 하네스 인터페이스 전용 디자인 언어 (2026-09-24)** | 에이전트 하네스 UI 표면 전용 최초 디자인 언어. 런타임 무관. 7만 별점이 L6 레이어가 자체 표준을 갖춘 성숙한 규율로 진화 중임을 시사. "에이전트 인터페이스 가독성" 패턴 첫 번째 시그널. | L6 주 / L2 부 |
| **treg ⭐3,152 + HarnessRouter ⭐2,600 — 오픈 프로토콜 툴 라우팅 (2026-09-25)** | 두 독립 도구가 MCP/REST/네이티브 SDK를 단일 인터페이스로 라우팅 — treg 데코레이터 기반 레지스트리, HarnessRouter Unified Harness Protocol. "오픈 프로토콜 라우팅 추상화" 서브타입 동일 날 2-시그널 충족. | L2/L3 주 |
| **Fosowl/agenticSeek ⭐27,305 — 완전 로컬 자율 에이전트 (2026-09-24)** | 멀티 스텝 자율 태스크(웹 탐색, 코드 실행, 파일 접근)를 온디바이스에서 실행. 외부 API·텔레메트리 없음. 스캔 코퍼스 최고 별점 완전 로컬 자율 에이전트. | L1 주 / L2 부 |
| **0xPlaygrounds/rig ⭐8,700 — Rust 네이티브 LLM 프레임워크 v0.36.0 (2026-09-25)** | 최초 추적 Rust 네이티브 L1 에이전트 런타임 — 20개+ 공급자 추상화, WASM 브라우저 배포, 카세트 녹화/재생. `hardware: browser-wasm` 축 후보 첫 번째 시그널. 974 포크. | L1 주 / L5 부 |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (851개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
## 🔥 지금 가장 뜨거운 것들 (2026-09-23)

| 신호 | 왜 중요한가 | 레벨 |
|------|------------|------|
| **AgentJev-0.6B ⭐266 + JevBench — Jev 패턴 6번째 시그널 (2026-09-23)** | malevrigns/agent-jev: Qwen3-0.6B에서 언어 모델 헤드 제거 후 순열 등변 스코어링 헤드로 교체 — 텍스트를 생성하지 않고 확률 분포를 출력. 불리언·다중 옵션·서수 세 가지 결정 기본형, ~50ms 순전파, 79.25% top-1 정확도. Apache-2.0 + 문서화된 아키텍처 + 공개 벤치마크를 갖춘 첫 번째 Jev 시그널. | L1 주 / L4 부 |
| **Stripe Knowledge AI Platform (Kai) — 엔터프라이즈 에이전트 아키텍처 레퍼런스 (HN 122점, 2026-09-23)** | LangChain deepagents + Kubernetes 세션별 샌드박스 위에 구축; 자체 서비스형 AgentStudio 제어 플레인으로 도메인 팀이 직접 에이전트 제작. 1,000+ 내부 툴·스킬, 5,000+ 일일 세션, 최대 932턴 세션 깊이 — 세 가지 신규 축 후보 도출. | L2 주 / L3 부 |
| **Unreal Agent — 비동기 툴 디스패치 하네스 (HN 115점, 2026-09-23)** | 코드 에이전트용 비동기 툴 디스패치 하네스; Codex 대비 40% 비용 절감 주장, Sequoia+First Round 투자. | L2 주 |
| **NobodyWho ⭐~1,200 — Flutter/RN/Godot용 온디바이스 LLM 추론 엔진 (2026-09-23)** | Rust 코어, Metal/Vulkan/CPU 자동 선택; 게임 엔진과 크로스플랫폼 모바일 프레임워크를 타겟하는 첫 번째 추적 추론 엔진. 툴 호출·STT·TTS·RAG 온디바이스 지원; EUPL-1.2. "Jev in 25 Lines of Python" 블로그(HN 513점)가 NobodyWho를 Jev 배포 기반으로 시연. | L1 주 / L7 부 |
| **Drop ⭐137 — 루트리스 에이전트 샌드박스 (2026-09-22)** | 커널 네임스페이스 기반 루트 권한 불필요 컨테이너 런타임 — 자격 증명 없이 에이전트 코드 격리; 에이전트 샌드박스를 위한 첫 번째 명시적 루트리스 런타임 추적. | L7 주 |
| **BuilderIO/agent-native ⭐5,188 — 에이전트 네이티브 UI 프레임워크 (2026-09-21)** | 에이전트가 인간 인터페이스 없이 직접 조작 가능한 UI 렌더링; 에이전트-퍼스트 인터페이스 레이어의 두 번째 신호. | L2 주 / L6 부 |
| **ConvAI RL-NAR — RL 학습 내러티브 에이전트 (HN 1,057점, 2026-09-20)** | RL 기반 내러티브 에이전트 런타임; 게임 내 Jev 패턴의 다섯 번째 시그널. | L1 주 |
| **Meta Muse 제로데이 — L3 보안 시그널 (2026-09-22)** | 미문서 설정 키를 통한 상업용 AI 에이전트 딕테이션 하이재킹; Patrick Wardle (Objective-See) 공개 + PoC; Meta 수 시간 내 패치. | L3 주 |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (833개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
...
| 날짜 | 변경 내용 |
|------|----------|
| 2026-09-26 | 3개 문서: Archify L6 (GeekNews — 코드베이스/설명에서 인터랙티브 아키텍처 다이어그램 생성; "에이전트 인터페이스 가독성" 크로스-데이 패턴 잠재적 4번째 시그널); Ollaya L1/L2 (HN 300점 — Jev/타입 결정 모델 로컬 서빙 데몬; 7번째 크로스-데이 Jev 시그널, 결정 모델 전용 최초 로컬 런타임); OpenAI 에이전트 HF 침해 L5/L3 (HN 114점 — 자율 에이전트가 의도된 기능으로 HuggingFace 인프라 침해; **3번째 크로스-데이 "상업용 AI 에이전트 공격 표면" 시그널** — 정식 승격 기준 충족). reference-levels.md: "상업용 AI 에이전트 공격 표면" 패턴 정식 승격. 레지스트리: 추가 없음. 50/50 테스트. |
| 2026-09-25 | 5개 문서 (보충 스캔): Ando L6/L2 (ando.so, 시드 $2,000만 — "에이전트 네이티브 비동기 커뮤니케이션 패브릭" 최초 추적; 에이전트가 아이덴티티+인박스 보유 팀원으로 참여); HarnessRouter ⭐2,600 L2/L3 (UHP 레퍼런스 구현 v0.11.2 — treg와 함께 "오픈 프로토콜 라우팅" 동일날 2-시그널 조건 충족); devdotfast/whiteboard ⭐1,100 L6/L2 (YC W26, HN 374점 — 에이전트-인간 디자인 캔버스 IDE, "에이전트 인터페이스 가독성" 크로스-데이 패턴 2번째 시그널); 0xPlaygrounds/rig ⭐8,700 L1/L5 (Rust 네이티브 LLM 프레임워크 v0.36.0 — 최초 추적 Rust L1 런타임, WASM 지원; `hardware: browser-wasm` 축 후보); androoAGI/starnet ⭐386 L2/L6 (로컬 퍼스트 데스크톱 에이전트 하네스, 픽셀아트 — "에이전트 인터페이스 가독성" 3번째 시그널). reference-levels.md: 보충 스캔 요약 + 두 개 📡 디스커버리 로그. 레지스트리: 추가 없음. 50/50 테스트. |
