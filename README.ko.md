...
## 🔥 지금 가장 뜨거운 것들 (2026-09-06)

| 신호 | 왜 중요한가 | 레벨 |
|------|------------|------|
| **[maziyarpanahi/openmed ⭐5.2k](https://github.com/maziyarpanahi/openmed) (GitHub Python 트렌딩, 2026-09-06)** | 로컬 퍼스트 헬스케어 AI: 2,200+ 의료 모델, HIPAA 제로 유출 보장(환자 데이터 기기 밖 전송 없음), MCP 서버, FastAPI REST, iOS/Android/브라우저 바인딩. 비용·레이턴시가 아닌 규제 컴플라이언스가 로컬 추론의 주요 설계 동인인 최초 도구. HF 2026 스프링 오픈소스 현황 1위 조직. | L1 주 / L4 부 |
| **[sooryathejas/METATRON ⭐3.9k](https://github.com/sooryathejas/METATRON) (GitHub Python 트렌딩, 2026-09-06)** | Ollama 로컬 LLM(metatron-qwen, 16k 컨텍스트)을 사용한 AI 침투 테스트 도구. nmap, whois, whatweb, nikto, CVE 조회를 실행해 결과를 로컬 모델에 전달, 취약점 분석. 제로 유출: 대상 IP·배너·자격증명이 클라우드 엔드포인트에 도달하지 않음. openmed와 함께 "컴플라이언스 기반 로컬 추론" L1 동기의 두 번째 같은 날 시그널. | L1 주 / L2 부 |
| **[experientiallabs/experiential ⭐1.8k +568★ 오늘](https://github.com/experientiallabs/experiential) (GitHub 트렌딩, 2026-09-06)** | 적응형 라우팅을 갖춘 모델 게이트웨이: 사용자·에이전트별 지출 한도를 런타임에 강제; 프로덕션 트래픽이 시간이 지남에 따라 라우팅 정책을 학습하는 라우터를 훈련. 단일 OpenAI 호환 엔드포인트 뒤에 폐쇄형·오픈소스·로컬·커스텀 모델 지원. 이 로그 최초의 학습 정책 기반 라우팅 시그널 — 정적 라우팅(freellmapi), 구독 풀링(CLIProxyAPI), 훅 기반 위임(Spotify Portal)과 구별. | L2 적응형 모델 게이트웨이 |
| **[XHToken/Spark-X2.5-4B](https://huggingface.co/XHToken/Spark-X2.5-4B) (HF 트렌딩, 2026-09-06)** | 하이브리드 어텐션(블록당 전체 어텐션 1개 + 슬라이딩 윈도우 3개)으로 1M 토큰 네이티브 컨텍스트를 지원하는 컴팩트 4B 오픈 웨이트 모델(1.7B 버전도 제공). MOPD 다중 교사 증류, 200+ 언어, 20조 토큰 사전 학습. 이 로그 최초로 Claude Code/Codex/OpenClaw/Hermes를 아키텍처 동기로 명시한 모델. | L1 컴팩트 에이전트 특화 모델 |
| **[OKF Agent Memory](https://github.com/okf-memory/okf-agent-memory) (HN 첫 페이지, 2026-09-06)** | 코딩 에이전트용 BM25 키워드-그래프 MCP 메모리: Google OKF v0.2 스키마, 300μs 이하 검색, Go 제로 의존성, 계층적 인덱스(80% 컨텍스트 절감). `memory_model` 설계 축의 세 번째 확인 시그널(Lemmalog + memoryfield + OKF). 검색 시 임베딩 모델 호출 없는 최초 메모리 아키텍처 — 쿼리당 추론 비용 제로. | L5 주 / L4 부 |
| **[AI 에이전트가 프론트엔드를 강타하고 있다 (Nolan Lawson)](https://nolanlawson.com/2026/08/23/the-asteroid-currently-hitting-frontend-web-development/) (GeekNews, 2026-09-06)** | 전 Microsoft Edge 엔지니어: AI 에이전트가 프론트엔드 업무를 충분히 잘 수행하면 개발자가 연습 루프를 건너뛰어 에이전트 오류를 감지하는 능력을 잃는 악순환이 생긴다고 주장. 네 영역(조직·코딩·SRE·프론트엔드)에 걸쳐 "에이전트 역량 침식" 패턴이 정식 인정되는 네 번째이자 마지막 시그널. | L7 주 / L6 부 |
| **[Anthropic 페르마의 마지막 정리 / P2M](https://github.com/anthropics/fermats-last-theorem) (HN 707 pts, 2026-09-05)** | 다중 에이전트 P2M 하네스가 Lean 4로 페르마의 마지막 정리 증명: 29,511개 정리, 1,300만 줄, 11일, ~60억 토큰. DAG 에이전트 조율 — 리프 노드 병렬 작업, 결과 트리 전파, 중복 경로 제거. 3계층 외부 검증. 장기 증명 가능 태스크에서 에이전트 플릿 조율의 새로운 패턴. | L2 주 / L3 부 |
| **[Spotify Portal](https://engineering.atspotify.com/2026/9/portal-by-spotify-cut-my-claude-code-token-usage-by-90) (HN 219 pts, 2026-09-05)** | Claude Code 내 세 가지 오픈 프리미티브(PreToolUse 훅, 배시 스크립트, 스킬 마크다운)로 프로덕션 모델 라우팅 구현. Java 모노레포에서 90% 토큰 절감. 에이전트가 임시 서버리스 런타임의 선언적 "모드"로 실행 — 새 프레임워크 없이 기존 Claude Code 프리미티브 조합. | L2 모델 라우팅 하네스 |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (744개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
