...
## 🔥 지금 가장 뜨거운 것들 (2026-08-28)

| 신호 | 왜 중요한가 | 레벨 |
|------|------------|------|
| **[tencent/Hy4-preview ⭐202](https://github.com/Tencent-Hunyuan/Hy4-preview) (텐센트, 2026-08-28 오픈소스 공개)** | 770B 전체 / 49B 활성화 MoE, 1M 토큰 컨텍스트, Apache 2.0 오픈소스로 오늘 공개. Gated DSA + IndexCache (크로스 레이어 희소 인덱스 재사용)·identity Hyper-Connections·내장 투기적 디코딩(MTP 레이어). 이번 주 세 번째 프론티어 오픈웨이트 모델(Qwen3.8-Flash-Next, GLM-5.3-Flash에 이어) — 오픈웨이트 프론티어가 일회성 이벤트가 아님을 확인. `weights: [proprietary | open]` 스키마 갭 강화. | L1 기반 모델 — 오픈웨이트 프론티어 |
| **[zai-org/GLM-5.3-Flash](https://huggingface.co/zai-org/GLM-5.3-Flash) (Zhipu AI, 2026-08-27, 1,470 HF 좋아요)** | 320B 전체 / 18B 활성화 MoE — GLM-5.3(744B/40B)과 별개 모델. GLM-5 시리즈 최초 네이티브 멀티모달; 하이브리드 희소+선형 어텐션으로 장문 컨텍스트 비용 절감; Manifold-Constrained Hyper-Connections(mHC); GLM-5.2 대비 1/10 가격(자체 발표). MIT 라이선스. `modality: [text | text+vision]` 스키마 갭. 독립 벤치마크 확인 필요. | L1 기반 모델 — 저가 프론티어 티어 |
| **[BDH-CQ: 순환 잠재 추론](https://arxiv.org/abs/2608.09888) (arXiv, HF 760 추천, 2026-08-10)** | 150M 파라미터 모델이 ARC-AGI-1 비용-정확도 파레토 프론티어를 과제당 $0.0007(0.07센트)에 돌파. 시연 예시가 순환 메모리를 업데이트하고 언어화된 chain-of-thought 없이 잠재 공간 반복 계산으로 쿼리 해결. 구조적 추론에 대형 모델이 필수라는 가정에 도전. `inference_mechanism: [autoregressive | recurrent-latent]` 스키마 갭. | L1 연구 — 경량 추론 모델 |
| **[tashfeenahmed/freellmapi ⭐21,473](https://github.com/tashfeenahmed/freellmapi) (GitHub Trending, 2026-08-28)** | 34개 LLM 프로바이더(구글·Groq·Cerebras·Mistral·Zhipu·NVIDIA·Cloudflare·Cohere 외 26개)의 무료 티어를 하나의 OpenAI 호환 엔드포인트로 통합. 월 74억 토큰, 6가지 라우팅 전략, 자동 페일오버, AES-256-GCM 키 암호화, OpenAI/Anthropic/Gemini/Ollama API 동시 에뮬레이션. Claude Code·Cline·Aider·Continue와 호환. clawfit의 현재 스코어링 로직에 `budget: 0` 갭 노출. | L7 인프라 — 무료 티어 LLM 집계기 |
| **[JetBrains/go-modern-guidelines ⭐2,533](https://github.com/JetBrains/go-modern-guidelines) (GitHub Trending, 2026-08-28)** | JetBrains 공식 프로젝트, AI 코딩 에이전트의 현대 Go 작성을 위해 설계. 두 가지 실패 모드 명명: "학습 데이터 지연"(에이전트가 최신 Go 기능 미인지)과 "빈도 편향"(에이전트가 훈련 데이터에서 더 자주 본 구형 패턴 선택). Go 1.0–1.27 관용구 커버, `go.mod` 인식 버전 조건부 조언. Python·Rust·TypeScript 커뮤니티로 확산될 템플릿. | L4 스킬 — 언어 최신성 참조 |
| **[Trail of Bits: VM이 AI 에이전트를 격리 못 한다](https://blog.trailofbits.com/2026/08/26/vms-wont-contain-cyber-capable-agents/) (HN 71점, 2026-08-26)** | QEMU/KVM VM 탈출 세 가지 방식 시연: CVE 악용, 취약점 체인, 제로데이 트리플. Firecracker 마이크로VM이 상대적으로 강한 격리. `execution_isolation` 스키마 갭. | L7 보안 — VM 격리 |
| **[Qwen3.8-Flash-Next](https://huggingface.co/Qwen/Qwen3.8-Flash-Next) (HN 443점, 2026-08-26)** | Alibaba Qwen4 프리뷰: 125B/6B MoE, GDN+QSA 하이브리드, 262k 컨텍스트, 플래그십 12배 저렴. "저가 프론티어 티어" 패턴의 첫 신호 — 이번 주 GLM-5.3-Flash·Hy4-preview로 세 신호 확인. | L1 기반 모델 — Qwen4 프리뷰 |
| **[Apple M6 및 M5 Ultra](https://apple.com) (HN 918점, 2026-08-26)** | M5 Ultra: 192–384 GB 유니파이드 메모리로 로컬 대형 모델 지원. MLX·LM Studio·Ollama 즉시 타겟팅. 오프라인/기밀 데이터 워크로드 수혜. | L1 로컬 하드웨어 서브스트레이트 |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (696개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
