...
| **Exfiltrate Your Weights — API 제공사 모델 가중치 추출 기법 (HN 600점, 2026-09-21)** | API 전용 추론 제공사에서 모델 가중치를 추출하는 구체적 기법을 문서화 — "클라우드 API = 모델 기밀성 보장"이 신뢰할 수 없음을 실증. ZCode(2026-09-19)의 반대 방향: ZCode는 에이전트가 사용자 데이터를 유출; 이건 공격자가 벤더 모델 가중치를 유출. 클라우드 추론 양방향 보안 위험 두 번째 독립 신호. | L3/L5 거버넌스 |
| **BuilderIO/agent-native ⭐5,188 — 풀스택 UI+에이전트 공유 액션 레이어 (GitHub 트렌딩, 2026-09-21)** | 5천 스타 임계값 돌파: 에이전트와 UI가 동일한 액션 정의 공유 — 도구 스펙과 UI 핸들러의 이중 유지보수 불필요. Builder.io는 프로덕션 CMS 회사. 에이전트가 UI 상태를 직접 구동해야 하는 풀스택 개발자 대상. | L2 주 / L6 부 |
| **ZCode 프라이버시 사건 — 코딩 에이전트가 git 히스토리를 클라우드로 무단 업로드 (HN 247점, 2026-09-19)** | 추적 중인 L1 코딩 에이전트가 세션 시작 시 전체 git 워크스페이스 히스토리를 Zhipu AI 클라우드로 자동 업로드. 추적 에이전트 최초 확인된 프라이버시 사건; `data_sensitivity: confidential` 필터 강화. | L1 프라이버시 |
...
| **JustVugg/colibri ⭐33,797 — 순수 C 로컬 MoE 추론 엔진 (GitHub 트렌딩, 2026-09-16)** | 포워드 패스마다 활성 전문가 샤드만 디스크에서 스트리밍. 양자화 없이 16 GB 노트북에서 float16 프론티어 MoE 모델 실행. 외부 의존성 없음. 로컬 점수 산정의 `vram_gb` 한계를 구속 조건으로 보는 시각에 도전. | L0 주 |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (826개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
