...
| **Bend — AI 코딩 실수를 막는 증명 검증 언어 (HN 225점, 2026-09-18)** | 언어 레벨에서 형식 증명으로 정확성을 강제 — AI 생성 코드 오류에 대한 명시적 대안으로 포지셔닝; CPU·GPU 실행 지원. 하네스·평가 레이어가 아닌 언어 레이어에서 AI 코딩 실수를 해결하는 첫 추적 도구. | L1/L3 주 |
| **Tencent/WeKnora ⭐25,259 — 조직 전체 RAG용 자체 호스팅 LLM 지식 플랫폼 (GitHub 트렌딩, 2026-09-17)** | 대형 랩에서 공개한 엔터프라이즈급 프라이빗 지식 베이스; 온프레미스 Go 배포로 데이터 유출 없음; 다운스트림 에이전트가 검색 API로 쿼리. 기밀 엔터프라이즈 데이터 대상 L5 지식 레이어 툴의 두 번째 시그널. 레지스트리 등록 완료. | L5 주 / L4 부 |
...
전체 분석: [`docs/research-watch/`](docs/research-watch/) (812개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
