...
| **[PrimeIntellect/prime-agent ⭐6,000](https://github.com/PrimeIntellect-ai/prime-agent) (GitHub Trending 전체 언어 1위, 2,271 일간, 2026-08-07)** | 자기 개선 RLM 코딩 에이전트(TypeScript/Python, v0.7.0). 두 가지 구조 프리미티브: RLM(컨텍스트를 변경 가능 변수로 처리) + Continual Harness(세션 간 영속 하네스 상태). `/refine` 명령으로 궤적을 검토하고 하네스 상태를 증거 기반으로 자동 갱신 — 베이스 시스템 프롬프트 미수정. 데몬 영속, P2P 에이전트 간 통신, ACP 모드. prime-rl(RL 훈련 인프라)과 별개 프로젝트. | L1/L2 자기 개선 RLM 에이전트 |
| **[Cloudflare Kitesurf](https://blog.cloudflare.com/kitesurf/) (HN 88점, 2026-08-07)** | AI 에이전트 전용 브라우저(Cloudflare Workers 위 구동, Rust/WASM: Blitz, Stylo CSS, Boa JS 엔진). CDP 호환 — Puppeteer/Playwright는 `browser=kitesurf` 파라미터 추가만으로 전환. Chromium 대비 CPU 3.1–3.8배, 메모리 4.7–7.0배 절약; 벽시계 시간은 1.7–1.8배 느림(의도적 절충). 215k+ WPT 통과. 베타 무료. 오픈소스 예정. 코퍼스 최초 에이전트 전용 브라우저 인프라. | L4/L7 에이전트 전용 브라우저 기판 |
| **[wshobson/agents ⭐38,600](https://github.com/wshobson/agents) (GitHub Trending Python, 2026-08-07)** | 멀티 하네스 플러그인 마켓플레이스: 단일 Markdown 소스에서 Claude Code·Codex CLI·Cursor·OpenCode·Gemini CLI·GitHub Copilot 네이티브 아티팩트 동시 생성. 94개 플러그인/203개 에이전트/175개 스킬/109개 명령어/16개 오케스트레이터. `plugin-eval` 품질 프레임워크(정적+LLM-심판+몬테카를로). 플러그인 메타데이터에 모델 티어 전략(Fable 5→Opus→Sonnet→Haiku) 명시. 코퍼스 최초 크로스 하네스 어댑터 빌드 시스템. | L4 크로스 하네스 플러그인 마켓플레이스 |
| **[semantica-agi/semantica ⭐2,255](https://github.com/semantica-agi/semantica) (GitHub Trending Python, 2026-08-07)** | "에이전트를 위한 오픈소스 팔란티르." 그래프 네이티브 컨텍스트 인프라(RDF + LPG 폴리글롯 백엔드). `record_decision()`으로 모든 AI 결정을 쿼리 가능 객체로 저장; `trace_decision_chain()`으로 인과 계보 추적; W3C PROV-O 형식으로 규제 감사 수출. LLM 없이 결정론적 추론(순방향 연쇄·Rete·Datalog·SPARQL). 퍼스트클래스 MCP 서버(15개 이상 도구). v0.6.0 2026-07-21. L5 컴플라이언스 프로브넌스 최초 시그널. | L5 그래프 네이티브 컨텍스트 + 컴플라이언스 프로브넌스 |
| **[Discovery Loop](https://discoveryloop.com) (HN 536점, 2026-08-06)** | Jeff Dean, Sanjay Ghemawat, Quoc Le, Oriol Vinyals가 구글 떠나 공동 창업 — 과학 실험 루프(제안→실행→학습) 수천 건 동시 자동화. 코퍼스 최고 자격 창업팀 + L5(자동화 연구 루프) 가장 직접적인 구현. 사전 제품 단계. | L5 자동화 연구 루프 |
| **[Atlassian Rovo 프롬프트 인젝션](https://www.promptarmor.com/resources/atlassian-rovo-exfiltrates-data) (HN 155점, 2026-08-06)** | 제로클릭 간접 프롬프트 인젝션으로 조직 레벨 웹 검색 제어 우회 및 Jira·Confluence 커넥터 데이터 유출. 코퍼스 최초 실제 기업 에이전트 침해 사례. | L3 기업 에이전트 보안 |
| **[google-gemini/gemini-cli ⭐106,400](https://github.com/google-gemini/gemini-cli) (GitHub Trending, 2026-08-06)** | 구글 공식 오픈소스 터미널 에이전트: Gemini 3 모델, 1M 컨텍스트, 네이티브 구글 검색 그라운딩, MCP 클라이언트. 무료 티어: 1,000 req/일. Meta Muse Code와 하이퍼스케일러 터미널 에이전트 2-시그널 클러스터 확인. | L1 구글 네이티브 터미널 에이전트 |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (590개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
