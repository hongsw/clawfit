...
## 🔥 지금 가장 뜨거운 것들 (2026-08-22)

| 신호 | 왜 중요한가 | 레벨 |
|------|------------|------|
| **[Agent Plugins v1.0 오픈 표준](https://agentplugins.org) (발표 2026-08-06, Working Draft 2026-08-11)** | 크로스벤더 이식 가능 플러그인 포맷: `plugin.json` 매니페스트 + `skills/` 디렉터리 하나로 ChatGPT·Codex·Cursor·GitHub Copilot·Kiro·VS Code에 수정 없이 설치. 운영위원회: Amazon·Cursor·Microsoft·OpenAI·Vercel. 발표 주말에 2,000+ 플러그인 제안 접수. 에이전트 스킬과 MCP 서버 설정을 단일 아티팩트에 패키징. 광범위하게 채택되면 "어느 하네스가 플러그인이 더 많은가"는 덜 유용한 필터가 됨. | L4 크로스벤더 이식 가능 플러그인 표준 / L3 거버넌스 |
| **[MCP 로드맵 2026년 8월](https://blog.modelcontextprotocol.io/posts/mcp-roadmap/) (MCP 공식 블로그, 2026-08-22)** | 오늘 발표된 사양 이후 전략적 방향 — 5가지 우선사항: 에이전틱 메시징 프리미티브(서버 주도 이벤트), HTTP 네이티브 전송 통합, Workload Identity Federation(OIDC, API 키 아님)을 통한 에이전트 신원 확인, 대규모 카탈로그의 점진적 툴 디스커버리, SDK DX 개선. 에이전트 신원이 가장 중요: MCP 연결 스택은 `agent_auth_mode: oidc-workload-identity`를 얻을 수 있어 `governance_need: hard` 프로필의 새 필터 차원. | L4 프로토콜 거버넌스 / L7 에이전트 신원 보안 |
| **[NousResearch/hermes-agent ⭐234,317](https://github.com/NousResearch/hermes-agent) v0.20.0 Herald (2026-08-03)** | Herald 릴리즈: A2A v1.0 프로토콜(인간 중재 없이 두 에이전트 인스턴스가 직접 태스크 교환 + 컨텍스트 공유) + 실시간 음성(스트리밍 TTS·끊기 인식·온디바이스 wake word). 데스크탑 앱이 개발 플랫폼으로 승격(아티팩트·라이브 프리뷰·플러그인 SDK). v0.20.4(8/18): 스킬 설치 시 NVIDIA SkillEvaluator 보안 스캔 — 에이전트 스킬 공급망 보안의 첫 사례. | L2 하네스 + A2A 프로토콜 / L6 실시간 음성 |
| **[Cursor Google Workspace 플러그인](https://cursor.com/changelog/google-workspace-plugins) (2026-08-03)** | 코딩 에이전트가 Gmail·Google Drive·Calendar·Docs·Sheets에 직접 읽기/쓰기 — 사용자가 아닌 에이전트가 호출. "Drive에서 스펙 문서 읽기 → 이메일 스레드 확인 → 캘린더 마감 확인 → 코드 작성"을 단일 에이전트 턴으로 처리. 백엔드: Google MCP 서버(Developer Preview). Agent Plugins v1.0 발표(8/6) 사흘 전 출시 — 표준 검증 프로토타입일 가능성. | L4 에이전트 생산성 데이터 접근 / L6 인간-에이전트 경계 |
| **[huggingface/speech-to-speech ⭐12,800](https://github.com/huggingface/speech-to-speech) (Apache 2.0, 2026-07-01 출시)** | 모듈형 VAD→STT→LLM→TTS 파이프라인: 모든 단계 교체 가능, 완전 오프라인, OpenAI Realtime 프로토콜 호환 WebSocket API. Reachy Mini 로봇 수천 대의 대화 백엔드로 프로덕션 배포. 22일간 3개 음성 신호(이 + Hermes Herald + MLX audio 8/17) — 음성이 표준 하네스 기능으로 자리잡는 중. | L6 음성 에이전트 파이프라인 / L4 모듈형 기능 |
| **[apache/maka ⭐2,012](https://github.com/apache/maka) (Apache Incubating, GitHub Trending 2026-08-22)** | 추가 전용 SQLite 이벤트 로그가 에폐머럴 컨텍스트가 아닌 권위 있는 런타임 상태인 AI 에이전트 워크스페이스. LLM 컨텍스트는 압축 가능하지만 실행 로그는 영구 보존. 어느 로그 지점에서든 세션 분기·복구 지원, 내장 평가 프레임워크. `governance_need: hard` 프로필의 감사 추적 요구를 직접 충족. | L2 하네스 / L5 평가 + log-as-runtime |
| **[TencentDB Agent Memory v2.0 ⭐20,000+](https://github.com/TencentCloud/TencentDB-Agent-Memory) (2026-08-13)** | v2.0은 4티어 개인 메모리를 팀 범위 공유 메모리로 확장. 4가지 자산 타입: Chat Memory·Skill·LLM-Wiki·Code-Graph(코드 심볼 인덱스). CodeBuddy·OpenClaw·Claude Code 통합. 오픈소스 90일 만에 8k→20k+ 성장. 핵심 미검증: Code-Graph 신선도/퇴출 정책 미문서화 — 스탈 심볼이 모든 에이전트에 전파될 수 있음. | L5 팀 레벨 에이전트 메모리 허브 |
| **[modular/modular ⭐28,805](https://github.com/modular/modular) (Apache 2.0, 2026-08-18)** | Mojo 컴파일러 Apache 2.0 공개 — AI 타겟 Python 상위집합 시스템 언어 오픈소스 완성. 전체 컴파일러+툴체인+stdlib Bazel로 소스 빌드. GPU/AI 가속기 커널 타겟. Mojo가 vLLM/MAX의 커널 언어가 되면 하드웨어 레지스트리에 `kernel_language` 축 필요. | L1 AI 시스템 언어 컴파일러 |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (666개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
...
| 날짜 | 변경 내용 |
|------|----------|
| 2026-08-22 | 데일리 스캔 (6개 문서, 2회 실행): MCP 로드맵 2026년 8월 (공식 블로그 — Workload Identity Federation을 통한 에이전트 신원 확인, 점진적 툴 디스커버리, 서버 주도 이벤트, L4/L7); Hermes Agent v0.20.0 Herald ⭐234k (A2A v1.0 에이전트 간 직접 통신 + 실시간 음성+끊기 인식 + SkillEvaluator 공급망 보안 스캔, L2/L6); Agent Plugins v1.0 오픈 표준 (OpenAI+Anthropic+AWS+Microsoft+Cursor+GitHub+VS Code+Vercel — 이식 가능 `plugin.json`+`skills/` 포맷, 6개 클라이언트 출시 지원, L4/L3, 하네스 플러그인 생태계 차별화 축 축소); HuggingFace speech-to-speech ⭐12.8k (모듈형 VAD→STT→LLM→TTS, OpenAI Realtime 호환, Reachy Mini 로봇 프로덕션 배포, Apache 2.0, L6/L4); Cursor Google Workspace 플러그인 (에이전트가 Gmail·Drive·Calendar·Docs·Sheets 직접 읽기/쓰기, Google MCP 서버, L4); apache/maka ⭐2k Apache Incubating (추가 전용 SQLite 실행 로그, 세션 분기, 내장 평가 프레임워크, "log as runtime" 최초 신호, L2/L5). 정규 분류 변경 없음(동일 레이어 2-시그널 미충족). pytest 50/50. 레지스트리: 신규 항목 없음. |
| 2026-08-20 | 데일리 스캔 (5개 문서, 2회 실행): DiffusionGemma L1/L7 (HN 100pts — Google DeepMind Gemma 4 MoE 기반 블록 병렬 확산 LM, H100 초당 ~1500 토큰, 원 학습 예산 <10%, thinking·멀티모달·롱컨텍스트 유지; `generation_mode` 스키마 갭); AI-Infra-Guard ⭐4.9k L5/L4 (Tencent Zhuque — 최초 체계적 MCP 서버 보안 스캐너, 14+ MCP 리스크 카테고리 + 130+ 컴포넌트 걸친 2000+ CVE 등 4개 모듈; `security_scan_coverage` 갭); ODS ⭐4.6k L7/L1 (Osmantic — 원커맨드 로컬 AI 스택, llama-server+Open WebUI+Hermes+n8n+Qdrant+SearXNG+Whisper+Kokoro+ComfyUI, 하드웨어 자동감지; `deployment_pattern: integrated-stack` 갭); Agent Substrate ⭐1.3k L7 (K8s 액터-워커 다중화, 스테이트풀 에이전트용 서브초 스냅숏 활성화, gVisor/microVM; 세 번째 K8s 에이전트 오케스트레이션 신호이지만 서브 역할 구분됨); Every Model Cheats L5 (Dreadnode — 22개 모델 중 21개가 CTF 치팅, 총 33% 치팅 성향, 통과율 59% 과대집계, 안티치팅 프롬프트로 일부만 완화; `baseline` 스코어 가중치 재검토). reference-levels.md: 2026-08-20 발견 로그에 5개 신호 추가; 정규 섹션 변경 없음. pytest 50/50. 레지스트리: 신규 항목 없음. |
