...
## 🔥 지금 가장 뜨거운 것들 (2026-08-29)

| 신호 | 왜 중요한가 | 레벨 |
|------|------------|------|
| **[NVIDIA-NeMo/labs-OO-Agents (NOOA) ⭐1,900](https://github.com/NVIDIA-NeMo/labs-OO-Agents) (NVIDIA, 2026-07-30, Apache 2.0)** | 에이전트 = 단일 Python 클래스. 필드는 상태, 메서드는 기능, 독스트링은 프롬프트, 타입 어노테이션은 계약. `...` 바디 메서드는 LLM 루프로 실행. Jupyter 스타일 REPL, AST 검증 + 모듈 차단 목록. GPT-5.5 기준 SWE-bench Verified 82.2%, 과제당 ~1.1M 토큰 / 28 호출(경쟁 프레임워크의 절반). v0.0.8 연구 알파, Python 3.12–3.13 필요. 프롬프트 템플릿·도구 레지스트리·오케스트레이션·타입 검증을 하나의 클래스로 통합. | L1/L2 — 객체지향 에이전트 런타임 |
| **[Google Gemini 3.5 Transcribe](https://deepmind.google/models/gemini/3-5-transcribe/) (Google, 2026-08-26)** | Chirp 3을 대체하는 전용 음성-텍스트 파운데이션 모델. 스트리밍 WER 4.0% / 비스트리밍 2.6%; Chirp 3 대비 최종 전사 완료 시간 70% 단축. 배경 소음·도메인 어휘(주문 ID, 우편번호, 파일명)·자기 수정 처리. Gboard Rambler·Gemini Live·Search Live 탑재; Google Cloud AI API 예정(가격 미공개). 에이전트 입력 레이어로서 추적된 첫 전용 ASR 모델 — `modality: [text | audio]` 스키마 갭 노출. | L1/L6 — ASR 파운데이션 모델 |
| **[agentconnect-md/agentconnect ⭐725](https://github.com/agentconnect-md/agentconnect) (2026-08-26, Apache 2.0)** | 오픈소스 "Claude Tag의 멀티에이전트 대안". Claude Code·Codex·Grok Build·DeepSeek·Pi 및 ACP 호환 에이전트를 Slack·GitHub·GitLab·Telegram·Discord·Lark를 통해 라우팅. 사람과 에이전트가 같은 대화 스레드 공유; 에이전트 간 상호 호출 및 세션 간 기억 유지. Docker Compose / Kubernetes 자체 호스팅. ACP(Agent Client Protocol) 호환성 주장 — 외부 스펙으로 미공개. L3 "플랫폼 네이티브 멀티에이전트 조율" 첫 신호. | L3 — 플랫폼 네이티브 멀티에이전트 협업 |
| **[ApodexAI/FrontierAgent ⭐1,300](https://github.com/ApodexAI/FrontierAgent) + [Apodex 1.1 논문](https://arxiv.org/abs/2608.23283) (HF 200 추천, 2026-08-24)** | 프론티어 시스템 기술 보고서와 함께 오픈소스화된 에이전트 런타임. FrontierAgent: ReAct(단일 에이전트) / Agent Team(코디네이터 + 병렬 서브에이전트) 이중 모드; 샌드박스 파일시스템(`/inputs` 읽기 전용, `/workspace`, `/outputs`); AgentOS로 교차 에이전트 작업 상태·출처 추적; Docker 미필수; 평가 스위트 포함. Apodex 1.1 Mini(35B 파라미터) 로컬 배포 가능 — HuggingFace 오픈 웨이트. 재무·과학 연구·수학·코딩·검색에서 선도 성능대 주장(독립 검증 필요). | L1/L2 — 프론티어 시스템 + 오픈소스 런타임 |
| **[tencent/Hy4-preview ⭐202](https://github.com/Tencent-Hunyuan/Hy4-preview) (텐센트, 2026-08-28 공개)** | 770B 전체 / 49B 활성화 MoE, 1M 토큰 컨텍스트, Apache 2.0. Gated DSA + IndexCache·iHC·내장 MTP 투기적 디코딩. 이번 주 세 번째 프론티어 오픈웨이트 모델 — 패턴으로 확인. `weights: [proprietary | open]` 스키마 갭 강화. | L1 기반 모델 — 오픈웨이트 프론티어 |
| **[zai-org/GLM-5.3-Flash](https://huggingface.co/zai-org/GLM-5.3-Flash) (Zhipu AI, 2026-08-27, 1,470 HF 좋아요)** | 320B/18B MoE, GLM-5 최초 멀티모달, 하이브리드 희소+선형 어텐션, GLM-5.2 대비 1/10 가격(자체 발표). MIT 라이선스. `modality` 스키마 갭. | L1 기반 모델 — 저가 프론티어 티어 |
| **[tashfeenahmed/freellmapi ⭐21,473](https://github.com/tashfeenahmed/freellmapi) (GitHub Trending, 2026-08-28)** | 34개 LLM 프로바이더 무료 티어를 단일 OpenAI 호환 엔드포인트로 통합. 월 74억 토큰, 6가지 라우팅, AES-256-GCM 키 암호화. `budget: 0` 갭 노출. | L7 인프라 — 무료 티어 LLM 집계기 |
| **[Apple M6 및 M5 Ultra](https://apple.com) (HN 918점, 2026-08-26)** | M5 Ultra: 192–384 GB 유니파이드 메모리. MLX·LM Studio·Ollama 즉시 타겟팅. 오프라인/기밀 데이터 워크로드 수혜. | L1 로컬 하드웨어 서브스트레이트 |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (700개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
...
| 날짜 | 변경 내용 |
|------|----------|
| 2026-08-29 | 데일리 스캔 (신규 4개, 오늘 총 5개): NOOA/labs-OO-Agents ⭐1.9k L1/L2 (에이전트=Python 클래스, SWE-bench 82.2%, Apache 2.0 연구 알파); Gemini 3.5 Transcribe L1/L6 (WER 4.0%, 첫 전용 ASR 에이전트 입력 레이어, `modality` 스키마 갭); agentconnect ⭐725 L3 (오픈소스 멀티에이전트 협업, ACP 호환, L3 플랫폼 네이티브 조율 첫 신호); ApodexAI/FrontierAgent ⭐1.3k L1/L2 + 논문 (35B 미니 오픈 웨이트, ReAct/Agent Team, AgentOS 출처 추적). 정규 분류 변경 없음; 50/50 테스트; 레지스트리 신규 항목 없음. |
| 2026-08-28 | 데일리 스캔 (5개 문서): tencent/Hy4-preview L1 (770B/49B MoE, 1M 컨텍스트, Apache 2.0, 오늘 오픈소스 공개 — 이번 주 세 번째 프론티어 오픈웨이트); zai-org/GLM-5.3-Flash L1 (320B/18B MoE, GLM-5 최초 멀티모달, 하이브리드 희소+선형 어텐션, mHC, GLM-5.2 대비 1/10 가격 — Hy4-preview와 저가 프론티어 티어 2-신호 확인); BDH-CQ L1 연구 (150M 순환 잠재 추론, 과제당 $0.0007 ARC-AGI-1 파레토 돌파, HF 760 추천); tashfeenahmed/freellmapi ⭐21.5k L7 (34개 프로바이더 무료 티어 통합, 월 74억 토큰, 6가지 라우팅 전략, `budget: 0` 갭 노출); JetBrains/go-modern-guidelines ⭐2.5k L4 (AI 코딩 에이전트용 공식 현대 Go 가이드라인, 학습 데이터 지연+빈도 편향 분석). 정규 분류 변경 없음; 50/50 테스트; 레지스트리 신규 항목 없음. |
| 2026-08-25 | 데일리 스캔 (5개 문서, 2회 실행): katanemo/plano ⭐7k+ L5/L2 (Envoy 기반 에이전트 프록시, LLM 라우팅+OTel 트레이싱+가드레일 필터 코드 수정 없이 중앙화, `routing_layer`/`observability_tier` 갭); CUDA→RISC-V L1 하드웨어 신호 (Hot Chips 2026 NVIDIA 공식, 12–18개월 지평, `risc-v-cuda` 새 하드웨어 티어 가능성); marin-community/marin ⭐2k L1 학습 서브스트레이트 (Stanford CRFM, DAG+Delphi 스케일링, 실패 실험 공개, `training_transparency` 갭); wanxingai/LightAgent ⭐1.2k L2 (LangChain 미사용 MCP 프레임워크, v0.10.0 이벤트 소싱 런타임, `dependency_weight`/`auditability` 갭); CelestoAI/SmolVM ⭐787 L2 (오픈소스 Firecracker/QEMU/libkrun 마이크로VM, `execution_isolation` 갭). 2-신호 규칙 미충족; 정규 분류 변경 없음. 50/50 테스트. 레지스트리: 신규 항목 없음. |
| 2026-08-24 | 데일리 스캔 (7개 문서, 2회 실행): claude-obsidian ⭐11.7k L5/L4 (GitHub Trending, Obsidian 볼트 에이전트 지식 세컨드 브레인, 15개 스킬, 복리 capture→ground→connect→maintain 루프, 트랜잭션 기반 뮤테이션, LYT/PARA/Zettelkasten — kepano/obsidian-skills와 구별: Claude가 볼트 안으로 들어감); claude-plugins-community ⭐1.3k L4 (+490 오늘, Anthropic 공식 커뮤니티 플러그인 마켓플레이스, 보안 심사 야간 미러, 3계층 공식+역할별+커뮤니티, Apache-2.0); Labor0 L2/L3 (GeekNews, 50+ 동시 에이전트 세션, CodingTask+AgentTask 그래프 오케스트레이션, 자율 PR 라이프사이클, catch-up 요약, ~30× 비용 절감); TryCaspian/caspian-sdk ⭐871 L6 (Show HN, 9개+ 채널 단일 on_message, 채널별 네이티브 렌더링, 오버랩 정책 queue/debounce/drop/parallel, 650+ 테스트, 채널 전반 영속적 에이전트 정체성); VoltAgent/awesome-ai-agent-papers ⭐1.7k L5 ref (364+ 2026년 논문, 7개 카테고리, 툴링 95+보안 82=47%, 주간 arXiv); benchflow-ai/awesome-evals ⭐840 L5 (443+ 큐레이션 링크, PATTERNS.md CI 통합 평가, 11.6k 논문 깊이-4 크롤); Agent.md L4 (HN 123점, 컨텍스트 주입 패턴, Claude Code/Goose/Cline 세션 시작 주입, 컨텍스트 희석 완화). 2-신호 규칙 미충족; 정규 분류 변경 없음. 50/50 테스트. 레지스트리: 신규 항목 없음. |
| 2026-08-23 | 데일리 스캔 (5개 문서): Untrivial-ai/agent-orchestrator ⭐9.9k L2 (GitHub Trending — 26+ 코딩 에이전트 플릿 관리 IDE, 태스크별 워크트리 격리, 자율 CI/머지/PR 해소, 칸반 보드, `fleet_controller` 차원 갭); KunAgent/Kun ⭐6.2k L1/L2 (GitHub Trending — 로컬 퍼스트 Electron GUI+TUI 공유 런타임, MCP 네이티브, 멀티 LLM, `cli+gui` 하이브리드 서브타입 첫 시그널); google-research/paper-orchestra ⭐116 L3/L2 (arXiv 2604.05018 — 5개 전문 에이전트 팀 LaTeX 논문 자동 작성, PaperWritingBench, Google Cloud AI Research 공식 출시); Autolith ⭐2.1k L1 (HN 147점 — Rust WASM 브라우저 네이티브 L1 런타임, 설치 불필요, 코퍼스 최초 브라우저 제1급 에이전트 런타임); OzBrain ⭐4.1k L1/L2 (호주 정부 후원 오픈 에이전트 런타임, DOE Genesis에 이어 두 번째 정부 AI 이니셔티브 시그널). 2-신호 규칙 미충족; 정규 분류 변경 없음. 50/50 테스트. 레지스트리: 신규 항목 없음. |
...
| 2026-08-14 | 데일리 스캔 (5개 문서): deepseek-ai/deepseek-harness ⭐93.7k L2 (GitHub Trending — Cordis 플러그인-컴포지션 코딩 에이전트, 고정 코어 없음, 93.7k 스타·공식 릴리스 제로); GLM-5.3 L1 (HN 928점 — "창발적 사이버 역량"을 안전 위험이 아닌 제품 기능으로 마케팅한 최초의 추적 LLM); hugohe3/ppt-master ⭐46.8k L4b (GitHub Python Trending — 크로스 하네스 네이티브 PPTX 생성 에이전트 스킬, Claude/GPT/Gemini/Kimi 호환, 3.8k 포크, 최초 오피스 문서 L4b 시그널); holaboss-ai/holaOS ⭐7.1k L2/L6 (GitHub Trending — Claude Code+Codex+네이티브 에이전트 동시 실행 워크스페이스, 플레인파일 공유 메모리, HolaApps 마켓플레이스); Unsloth Desktop L7/L2 (GeekNews — Tauri 무료 오픈소스 앱, `unsloth start claude` 단일 명령으로 로컬 파인튜닝 모델을 Claude Code에 연결, PDF/CSV/JSON 노코드 파인튜닝). 정규 분류 변경 없음(2-시그널 규칙: 동일 날짜 패턴 쌍 미확인). 50/50 테스트. 레지스트리: 신규 항목 없음. |
| 2026-08-11 | 데일리 스캔 (5개 문서, 2회 실행): msitarzewski/agency-agents ⭐142k L4b (GitHub Trending #2 — 18+ 직군 230+ 역할-페르소나 에이전트 정의, 데스크탑 설치앱, 하네스 무관; 최초 "역할-페르소나" 서브타입 vs 태스크 지향 스킬팩); Needle2/CactusCompute L1 (HN 114점 — 14 MB 에이전틱 LLM, 스마트폰/웨어러블/로봇 대상, 네이티브 도구 호출, 코퍼스 내 최소 엣지 추론 풋프린트); Stoa Markets YC S26 (HN 62점 — 기관용 GPU/AI 하드웨어 마켓플레이스, H100-GB200 실시간 가격 발견, 검증된 거래 상대방); cathrynlavery/diagram-design ⭐6.3k L4b (GitHub 전체 언어 트렌딩 #1 오늘 +1,612 — 29개 편집 다이어그램 Claude Code 스킬, 웹사이트 브랜드 온보딩, WCAG AA, 경량 프로그레시브 디스클로저, "시각적 출력 템플릿 스킬" 최초 시그널); antirez/h3.c ⭐1.1k L7 (HN 396점 — 네이티브 C+Metal MiniMax H3 비디오 확산 Apple Silicon 추론, 코퍼스 내 비디오 확산 최초 사례, SSD 스트리밍, Metal 4 TensorOps). 정규 분류 변경 없음. 50/50 테스트. 레지스트리: 신규 항목 없음. |
| 2026-08-10 | 데일리 스캔 (6개 문서): Meta Muse Glimmer 30B L1 (HN 765점 — 최초 오픈 웨이트 에이전틱 특화 모델, Apache 2.0, 4-bit 18–20 GB, SWE-Bench 76%, 추론 깊이 제어 low/medium/high/xhigh, AMD·NVIDIA 동일날 하드웨어 가이드); Docker Sandboxes L7/L2 (HN 518점 — 마이크로VM 에이전트 격리, YOLO 모드 안전화, Docker Desktop 불필요, Docker AI Governance 조직 MCP 정책 시행); vitali87/code-graph-rag ⭐2.9k L4a/L4c (Memgraph 그래프 DB + MCP, AST 기반, 13+ 언어 — 세 번째 구조적 코드 메모리 신호); danielmiessler/LifeOS ⭐17.8k L2 (v6.0.0 2026-07, PAI에서 이름 변경, Synapse+Atlas+Ledger+Cortex 명명 서브시스템, Claude Code 주요 개인용 하네스); pingdotgg/t3code ⭐17.9k L6 (초기 단계, 벤더 무관 iOS/Android/웹/Electron 컨트롤 서피스 for Claude Code+Codex+Cursor+Grok Build+OpenCode, 비동기 모바일 권한 위임); AntigmaLabs/ante ⭐817 L2 (HN Show HN, Rust 단일 바이너리 오프라인 코딩 에이전트, llama.cpp 내장, 12+ LLM 공급자, 코퍼스 최초 단일 바이너리 Rust 하네스). MCP 거버넌스 두 방향 수렴(Docker + DoorDash 내부 게이트웨이). 정규 분류 변경 없음. 50/50 테스트. 레지스트리: 신규 항목 없음. |
| 2026-08-09 | 데일리 스캔 (5개 문서, 2회 실행): Claude Code 크로스 세션 메시징 (CC v2.1.224+, L4c/L2 네이티브 P2P 세션 간 메시징 — ListAgents/SendMessage; 이전에 필요했던 MCP 브리지 레이어 제거; Bedrock/GCP/Foundry 미지원); Airbnb eval-driven development (L5 방법론, "eval-as-engineering" 두 번째 크로스데이 확인); vectorize-io/hindsight ⭐19.4k L4a/L5 (생체모방 3-타입 에이전트 메모리 REST 서비스; Retain/Recall/Reflect; LongMemEval SOTA 주장; v0.9.0 8/7); openchamber/openchamber ⭐7.8k L2/L6 (OpenCode용 멀티모델 ADE; Fusion으로 최대 5개 병렬 모델 결과 자동 합성; Changes Walkthrough; Session Goals; Private Relay; MIT); agentscope-ai/ReMe ⭐3.3k L4a/L5 (Markdown 네이티브 에이전트 메모리; Auto Memory/Resource/Dream 파이프라인; Claude Code MCP; ACL 2026). 정규 분류 변경 없음. 50/50 테스트. 레지스트리: 신규 항목 없음. |
