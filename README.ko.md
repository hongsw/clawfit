...
## 🔥 지금 가장 뜨거운 것들 (2026-08-11)

| 신호 | 왜 중요한가 | 레벨 |
|------|------------|------|
| **[msitarzewski/agency-agents ⭐142k](https://github.com/msitarzewski/agency-agents) (GitHub Trending #2, 2026-08-11)** | 엔지니어링·디자인·마케팅·법무 등 18개 이상 직군에 걸친 230+ AI 에이전트 역할-페르소나 정의 — 각각 성공 지표와 도메인 워크플로우 포함. macOS/Linux/Windows 데스크탑 설치앱 제공. Claude Code·Copilot·Cursor·Aider·Windsurf 등 8+ 플랫폼에서 동작. 기존 L4b 스킬팩(태스크 지향)과 구조적으로 다름: 에이전트에 특정 태스크 스킬이 아니라 *정체성*을 부여하여 기본 동작과 우선순위를 형성. | L4b 역할-페르소나 에이전트 설정 라이브러리 |
| **[cathrynlavery/diagram-design ⭐6,285](https://github.com/cathrynlavery/diagram-design) (GitHub 전체 언어 트렌딩 #1, +1,612 오늘, 2026-08-11)** | Claude Code 스킬로 29개 편집 품질 다이어그램 타입 제공 — 자체 완결 HTML+SVG, 빌드 단계 없음, JS 런타임 없음, 외부 의존성 없음. 브랜드 온보딩: 웹사이트 읽기 → 팔레트+타이포그래피 추출 → 시맨틱 토큰 매핑 → WCAG AA 검증 → 60초 내 전체 적용. 경량 SKILL.md 인덱스 + 수요 기반 34개 타입별 참조 파일 로딩. 코퍼스 내 최초 "시각적 아웃풋 템플릿 스킬". | L4b 편집 다이어그램 Claude Code 스킬 + L6 시각적 출력 |
| **[antirez/h3.c ⭐1,139](https://github.com/antirez/h3.c) (HN 396점, 2026-08-11)** | antirez(Redis 창시자)가 작성한 Apple Silicon용 네이티브 C+Metal MiniMax H3(33B 영상+오디오 확산 모델) 추론 엔진. 텍스트→영상, 첫/마지막 프레임 조건부 생성, 참조 기반 생성(Ref2VA), 메모리 제한 환경용 SSD 스트리밍, M3/M5 Max용 Metal 4+TensorOps. Apple Silicon에서 비디오 확산 모델 추론 최초 추적 사례(기존: 텍스트 전용). M5 Max 128GB: 클립당 "수 분"; RTX Pro 6000: 2–3분 — 비디오 확산은 통합 메모리보다 전용 VRAM이 유리. | L7 Apple Silicon 비디오 확산 추론 서브스트레이트, L4c 영상 생성 액션 |
| **[Needle2 / CactusCompute](https://cactuscompute.com/) (HN "Show HN" 114점, 2026-08-11)** | 스마트폰·웨어러블·스마트홈·로봇용 14 MB 에이전틱 LLM — 코퍼스 내 최소 추론 풋프린트(직전 최소: $8 마이크로컨트롤러 2,890만 파라미터 모델). 네이티브 도구 호출·구조화 추출·디바이스 제어 지원; 에이전틱 워크플로우 전용 설계(채팅 모델 개조가 아님). 50 MB 미만 에이전틱 추론은 clawfit이 현재 모델링하지 않는 하드웨어 티어. | L1 엣지/임베디드 에이전틱 LLM 서브클러스터 |
| **[Stoa Markets (YC S26)](https://stoaexchange.com/) (HN "Launch HN" 62점, 2026-08-11)** | H100/A100/H200/B200/GB200 SKU 실시간 가격 발견, 검증된 거래 상대방, 구조화된 RFQ + 다단계 정산을 제공하는 첫 YC 지원 기관용 GPU/AI 하드웨어 마켓플레이스. 가격 불투명이 장벽인 기업 `hardware: cloud` 프로파일에 관련된 인프라 맥락. | L7 하드웨어 조달 인프라 |
| **[Meta Muse Glimmer 30B](https://research.meta.ai/blog/introducing-muse-glimmer-open-agentic-model) (HN 765점, 2026-08-10)** | 코퍼스 내 최초 에이전틱 워크플로우 훈련 오픈 웨이트 모델. 30B, 4-bit 18–20 GB(RTX 4090 적합). 추론 깊이 제어(low/medium/high/xhigh). 131k+ 컨텍스트. SWE-Bench Verified 76%. Apache 2.0. AMD·NVIDIA 동일 날짜 하드웨어 가이드. | L1 로컬 GPU 배포용 오픈 웨이트 에이전틱 특화 모델 |
| **[Docker Sandboxes](https://www.docker.com/blog/docker-sandboxes/) (HN 518점, 2026-08-10)** | 에이전트 실행 안전용 Docker 첫 네이티브 제품: 마이크로VM 격리로 "YOLO 모드" 자율성 제공. Docker Desktop 불필요. **Docker AI Governance**: 조직 전체 MCP 정책 시행. Claude Code·Gemini CLI·Codex·OpenCode·Kiro 지원. 동일 날 MCP 거버넌스 두 방향 수렴. | L7 마이크로VM 에이전트 실행 격리 + L2 조직 MCP 거버넌스 |
| **[vectorize-io/hindsight ⭐19,400](https://github.com/vectorize-io/hindsight) (GitHub Python Trending, 2026-08-09)** | 생체모방 3-타입 에이전트 메모리: 세계 사실·경험·정신 모델. Retain+Recall+**Reflect** — Reflect는 축적 메모리에서 새 인사이트 생성(단순 검색과 다름). 복합 검색: 시맨틱+BM25+그래프+시간. LongMemEval SOTA 주장. Ollama 오프라인 지원. v0.9.0 2026-08-07. | L4a/L5 생체모방 에이전트 메모리 REST 서비스 |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (612개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
