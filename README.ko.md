...
## 🔥 지금 가장 뜨거운 것들 (2026-08-17)

| 신호 | 왜 중요한가 | 레벨 |
|------|------------|------|
| **Stripe, OpenRouter를 $70억+ 인수 (Bloomberg/HN, 2026-08-17)** | 결제 인프라 기업이 주요 LLM 라우팅 익스체인지(300+ 모델, 월 100조 토큰)를 인수. OpenRouter의 중립성(공급자 미제휴) 유지 여부 불확실 — `governance_need: hard`, `data_sensitivity: confidential` 프로필의 컴플라이언스 위험 상승. 2026년 5월 시리즈 B 대비 3개월 만에 5× 밸류에이션 도약. | L7 M&A / 라우팅 거버넌스 |
| **[titanwings/colleague-skill ⭐23,124](https://github.com/titanwings/colleague-skill) (GitHub Trending, 2026-08-17)** | 전문가 지식을 배포 가능한 페르소나 스킬로 증류: 페르소나+업무 2레이어 추출로 특정인의 사고 패턴을 Claude Code·Hermes Agent·OpenClaw·Codex·DeepSeek Harness에서 실행되는 스킬 번들로 컴파일. 커뮤니티 스킬 100+, 기여자 165+, arXiv:2605.31264 학술 검증. 하네스 독립적 페르소나 스킬이라는 새로운 L4 하위 타입 최초 확인. | L4 페르소나 스킬 증류 |
| **[Blaizzy/mlx-audio ⭐7,746](https://github.com/Blaizzy/mlx-audio) (GitHub Trending Python, 2026-08-17)** | Apple Silicon용 통합 TTS/STT/음악 추론 라이브러리: TTS 모델 30+, STT 모델 15+, 3~8비트 양자화, OpenAI 호환 REST API, 스트리밍 TTS. oMLX(2026-08-16)와 결합 시 `hardware: local_mac` 프로필에서 클라우드 없이 완전 에이전트 음성 파이프라인 구성 가능. | L7 Apple Silicon 오디오 추론 |
| **[akitaonrails/ai-memory ⭐1,904](https://github.com/akitaonrails/ai-memory) (GitHub Trending, 2026-08-17)** | Rust 단일 바이너리: 코딩 에이전트 CLI 20+ 지원, SessionStart 핸드오프(Claude Code 중단 → Codex 시작 → 컨텍스트 그대로 이어받기). 위키를 git 버전 관리 마크다운으로 저장, FTS5+엔티티+그래프 이웃+선택적 벡터 혼합 검색. MCP 데몬 모드. '벤더 핸드오프' 문제를 해결하는 최초 도구. | L5 크로스 벤더 에이전트 메모리 |
| **[Speko — YC S26](https://speko.ai) (HN Launch HN, 2026-08-17)** | 음성 AI 모델 라우터: STT 모델 21+를 9개 언어별로 벤치마크($0.001~$0.016/분), 9개 언어에서 4개 모델이 각각 최적, 500ms 미만 페일오버, OpenAI 프로토콜, MCP 서버. OpenRouter의 LLM 라우터와 동일한 구조를 STT/TTS 레이어에 적용한 최초 독립 음성 전문 라우터. | L7 음성 AI 모델 라우터 |
| **[math-ai-org/MathCode ⭐622](https://github.com/math-ai-org/mathcode) (HN 프론트페이지, 2026-08-17)** | Lean 4 수학 형식화 전용 터미널 코딩 에이전트: 자연어 문제 → 정리 변환 → 반복 증명(쓰기→Lean LSP 오류→수정, 0.4초/검사), Mathlib 보조 정리 검색, 병렬 증명 전략. 일반 코드가 아닌 형식 검증된 수학적 증명이 주 산출물 — 새로운 `task: math-formalization` 하위 타입. | L2 수학 형식화 에이전트 |
| **[jundot/oMLX ⭐18,791](https://github.com/jundot/omlx) (GitHub Trending Python, 2026-08-16)** | Apple Silicon 추론 서버: RAM→SSD KV 캐시 계층화, 다중 모델 LRU 서빙, OpenAI+Anthropic API 단일 프로세스. `hardware: local_mac` 프로필의 유효 컨텍스트를 SSD 용량까지 확장. mlx-audio와 결합 시 완전 로컬 음성 에이전트 스택 구성. | L1/L7 Apple Silicon 멀티 모델 추론 |
| **[THUDM/slime ⭐8,059](https://github.com/THUDM/slime) (GitHub Trending Python, 2026-08-16)** | Megatron(학습)+SGLang(롤아웃)을 Data Buffer로 연결하는 RL 사후 학습 프레임워크. 멀티 에이전트 RL·도구 사용 학습·장기 태스크 지원, 델타 웨이트 동기화. GLM-4.7~5.2 프로덕션 검증. | L1 RL 사후 학습 프레임워크 |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (638개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
