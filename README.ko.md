...
## 🔥 지금 가장 뜨거운 것들 (2026-08-24)

| 신호 | 왜 중요한가 | 레벨 |
|------|------------|------|
| **[AgriciDaniel/claude-obsidian ⭐11,711](https://github.com/AgriciDaniel/claude-obsidian) (GitHub Trending, 2026-08-24)** | Obsidian 볼트를 에이전트 지식 세컨드 브레인으로 활용하는 로컬 퍼스트 도구. 15개 전문 스킬(capture, ground, connect, maintain)이 복리 지식 루프를 구성. Claude Code가 트랜잭션 기반 뮤테이션으로 볼트 내부에 직접 접근. LYT, PARA, Zettelkasten, 일반 파일링 지원. kepano/obsidian-skills(2026-04-06, 플러그인으로서의 스킬)와 구별: claude-obsidian은 Claude가 볼트 안으로 들어간다. `knowledge_vault`/`knowledge_format` 스키마 갭 확인. | L5/L4 로컬 지식 세컨드 브레인 |
| **[anthropics/claude-plugins-community ⭐1,275](https://github.com/anthropics/claude-plugins-community) (+490 오늘, GitHub Trending, 2026-08-24)** | Anthropic 공식 커뮤니티 플러그인 마켓플레이스 — 보안 심사 파이프라인의 야간 동기화 미러. 3계층 구조: 공식 + 역할별 + 커뮤니티. CLI: `claude plugin marketplace add <registry>`(서드파티 레지스트리 지원). 동반 저장소: claude-plugins-official + knowledge-work-plugins. Apache-2.0. 플러그인 마켓플레이스가 스킬팩과 구분되는 퍼스트클래스 역량 배포 채널로 부상 중임을 확인. | L4 플러그인 마켓플레이스 |
| **[Labor0](https://kdy1.dev/2026-8-22-labor0) — 멀티 에이전트 세션 코디네이터 (GeekNews, 2026-08-24)** | 50+ 동시 AI 에이전트 세션 관리 워크플로 플랫폼. 대형 결과물을 CodingTask(PR 단위)와 AgentTask로 분해하는 그래프 기반 오케스트레이터. CI 수정·머지 충돌 해소·리뷰 반영을 인간 개입 없이 처리하는 자율 PR 라이프사이클. Catch-up 요약 기능으로 마지막 체크인 이후 변경 사항을 압축 제공. 토큰 비용 약 30× 절감. | L2/L3 멀티 에이전트 코디네이터 |
| **[TryCaspian/caspian-sdk ⭐871](https://github.com/TryCaspian/caspian-sdk) (Show HN 2026-08-21, 2026-08-24)** | 9개+ 채널(Slack·Discord·Telegram·WhatsApp·Instagram·이메일·SMS·X·Linear)에 단일 `on_message` 핸들러 제공. 채널별 네이티브 렌더링(Slack Block Kit, Discord 임베드, Telegram 키보드). 선언형 오버랩 정책: `queue`, `debounce`, `drop`, `parallel`. 650+ 테스트. CopilotKit/channels-sdk(코파일럿 출력 라우팅)와 구별: Caspian은 채널 전반의 영속적 에이전트 정체성을 제공 — 에이전트가 인간을 따라간다. | L6/L4 멀티채널 에이전트 정체성 |
| **[VoltAgent/awesome-ai-agent-papers ⭐1,700](https://github.com/VoltAgent/awesome-ai-agent-papers) (웹 검색, 2026-08-24)** | 2026년 AI 에이전트 연구 논문 364+편을 7개 카테고리로 분류한 주간 업데이트 인덱스. Agent Tooling(95)+AI Agent Security(82)=전체의 47%. 83회 커밋, 181 포크, 매주 arXiv 업데이트. 프로덕션 에이전트 프레임워크가 12–18개월 내 추가할 역량 차원의 선행 지표. | L5 리서치 레퍼런스 |
| **[benchflow-ai/awesome-evals ⭐840](https://github.com/benchflow-ai/awesome-evals) (웹 검색, 2026-08-24)** | 443+ 큐레이션 링크, 143개 심층 읽기 노트. PATTERNS.md: LLM-as-judge·pass@k·오류 분석·CI 게이팅 등 실행 가능한 평가 패턴 제공. 11,600개 논문 깊이-4 인용 크롤. 벤치마크 무결성 섹션(오염·레이블 오류·리더보드 게이밍). `eval_methodology` 스키마 갭: 레지스트리 항목에 평가 방법론을 표현할 방법 없음. | L5 평가 방법론 |
| **[Agent.md 패턴](https://fabiensanglard.net/agent.md/index.html) (HN 123점, 2026-08-24)** | 영속적 컨텍스트 주입 패턴: 코딩 하네스(Claude Code·Goose·Cline)가 세션 시작 시 자동으로 로드하는 구조화된 마크다운 파일. 불변 지침(코딩 스타일·아키텍처 규칙·커밋 기준)을 앞쪽에 배치해 LLM 어텐션 가중치를 유지하는 컨텍스트 희석 완화 전략. 기능적으로 CLAUDE.md와 동일. HN 트랙션이 생태계 전반의 채택 관심을 시사. | L4 컨텍스트 주입 패턴 |
...
전체 분석: [`docs/research-watch/`](docs/research-watch/) (677개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
