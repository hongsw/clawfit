...
## 🔥 지금 가장 뜨거운 것들 (2026-08-10)

| 신호 | 왜 중요한가 | 레벨 |
|------|------------|------|
| **[Meta Muse Glimmer 30B](https://research.meta.ai/blog/introducing-muse-glimmer-open-agentic-model) (HN 765점, 2026-08-10)** | 코퍼스 내 최초로 에이전틱 워크플로우를 목표로 훈련된 오픈 웨이트 모델 — 도구 사용을 위해 파인튜닝된 범용 모델이 아니라 자율적 멀티스텝 작업 완료를 핵심 훈련 목표로 설정. 30B, 4-bit 양자화 시 18–20 GB(RTX 4090 적합). 추론 깊이 제어(low/medium/high/xhigh). 비전 인코더(~1.8B ViT-G/14). 131k+ 컨텍스트. SWE-Bench Verified 76%. Apache 2.0. AMD·NVIDIA 모두 동일 날짜 하드웨어 최적화 가이드 공개. | L1 로컬 GPU 배포용 오픈 웨이트 에이전틱 특화 모델 |
| **[Docker Sandboxes](https://www.docker.com/blog/docker-sandboxes/) (HN 518점, 2026-08-10)** | Docker의 에이전트 실행 안전 문제 첫 번째 네이티브 제품: 마이크로VM 격리(VM보다 빠르고 컨테이너보다 강력)로 "YOLO 모드"(`--dangerously-skip-permissions`) 완전 자율성 제공 — 호스트 노출 없이. 파일시스템·네트워크 폭발 반경 제어. Docker Desktop 불필요. **Docker AI Governance**: 조직 전체 MCP 서버 접근 정책 시행. Claude Code·Gemini CLI·Codex·OpenCode·Kiro 지원. 동일 날 MCP 거버넌스 도구 두 방향 수렴(Docker 샌드박스 + DoorDash 내부 게이트웨이). | L7 마이크로VM 에이전트 실행 격리 + L2 조직 MCP 거버넌스 |
| **[vitali87/code-graph-rag ⭐2,974](https://github.com/vitali87/code-graph-rag) (GitHub Python Trending, 2026-08-10)** | 코딩 에이전트용 모노리포 지식 그래프: Tree-sitter AST 파싱 → Memgraph(Cypher 호환 그래프 DB) → MCP 서버. 코드를 텍스트가 아닌 실제 호출 관계와 임포트로 인코딩. 13+ 언어 크로스. 실시간 인덱싱. gitnexus(WASM/TS, 31.5k★)·codebase-memory-mcp 이후 세 번째 구조적 코드 메모리 도구. | L4a/L4c 구조적 코드 메모리 MCP 서버, Python+Memgraph 서버사이드 변형 |
| **[danielmiessler/LifeOS ⭐17,783](https://github.com/danielmiessler/LifeOS) (GitHub Trending, 2026-08-10)** | 개인용 AI 하네스 (v6.0.0, 2026-07-02 — PAI에서 이름 변경): 명명된 서브시스템 — Synapse(입력 라우터)·Atlas(자산 그래프)·Ledger(변경 추적). Cortex 영속 메모리가 세션 간 축적되어 라우팅 결정에 영향. "The Algorithm v8.4.0" 별도 버전 관리. 하네스 무관이지만 Claude Code 주요. 코퍼스 내 최초 공식 명명 아키텍처를 가진 개인 범위 하네스. MIT. | L2 힐 클라이밍 아키텍처를 가진 개인용 AI 하네스 |
| **[pingdotgg/t3code ⭐17,940](https://github.com/pingdotgg/t3code) (GitHub Trending, 2026-08-10)** | AI 코딩 에이전트용 벤더 무관 크로스 디바이스 컨트롤 서피스: iOS/Android 앱 + 웹 앱 + Electron 데스크탑으로 Claude Code·Codex·Cursor·Grok Build·OpenCode 제어. 비동기 모바일 권한 위임 — 에이전트가 무인 실행 중 권한 요청 시 스마트폰에서 승인. 매우 초기 단계(명시적 "very very early" 경고). 특정 하네스 백엔드에서 UI를 분리하는 최초의 에이전트 컨트롤 서피스. | L6 벤더 무관 크로스 디바이스 에이전트 하네스 컨트롤 서피스 |
| **[AntigmaLabs/ante ⭐817](https://github.com/AntigmaLabs/ante) (HN "Show HN" 66점, 2026-08-10)** | Rust 코딩 에이전트, 단일 ~15 MB 바이너리, 런타임 의존성 없음, llama.cpp 내장(별도 프로세스 불필요) — GGUF 모델로 완전 오프라인 동작. 12+ LLM 공급자(Anthropic·OpenAI·Google·xAI·OpenRouter·로컬 llama.cpp). 2026-03-부터 퍼블릭 알파. Apache 2.0 소스. 코퍼스 내 최초 Rust 네이티브 단일 바이너리 코딩 에이전트 하네스. | L2 오프라인 지원 Rust 코딩 에이전트, 단일 바이너리 배포 |
| **[vectorize-io/hindsight ⭐19,400](https://github.com/vectorize-io/hindsight) (GitHub Python Trending, 2026-08-09)** | 생체모방 3-타입 에이전트 메모리 서비스: 세계 사실·경험·정신 모델. Retain·Recall·**Reflect** — Reflect는 기존 메모리 검색 대신 축적 메모리에서 새 인사이트 생성. 복합 검색: 시맨틱+BM25+그래프+시간 필터링. LongMemEval SOTA 주장. Ollama 오프라인 지원. REST API + Python/Node.js. v0.9.0 2026-08-07. | L4a/L5 생체모방 에이전트 메모리 REST 서비스 |
| **[openchamber/openchamber ⭐7,800](https://github.com/openchamber/openchamber) (HN 프론트 페이지, 2026-08-09)** | OpenCode용 ADE. **Fusion**: 동일 프롬프트를 최대 5개 모델에 동시 실행 후 최적 결과를 단일 diff 출력으로 자동 합성. Changes Walkthrough·Session Goals·Preview Inspection·QR 코드 Private Relay. 크로스 디바이스: Desktop/Web/VS Code/iOS/Android. MIT. | L2/L6 Fusion 합성 멀티모델 ADE |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (607개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
