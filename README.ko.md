...
## 🔥 지금 가장 뜨거운 것들 (2026-08-16)

| 신호 | 왜 중요한가 | 레벨 |
|------|------------|------|
| **[harry0703/MoneyPrinterTurbo ⭐104,390](https://github.com/harry0703/MoneyPrinterTurbo) (GitHub Trending, 2026-08-16)** | LLM 스크립팅 → 스톡 영상 → Whisper 자막 → TTS 오디오 → FFmpeg → TikTok/인스타그램/유튜브 자동 업로드까지 완전 자동화 AI 영상 제작 파이프라인 하네스. 12+ LLM 프로바이더; Ollama + LiteLLM 로컬 모델 지원. 인터페이스 4가지: WebUI·REST API·CLI·AI 에이전트 모드. 104k 스타는 AI 영상 자동화의 대중 수요를 입증. 코퍼스 최초 비디오 콘텐츠 제작 도메인 L2 하네스. | L2/L4b AI 비디오 파이프라인 |
| **[tinygrad/Chestnut eGPU dock](https://github.com/tinygrad/asm2464pd-firmware) — 오픈소스 펌웨어 (HN 109점, 2026-08-16)** | comma.ai/tinygrad가 $249 Chestnut eGPU 독($799 AMD RX 9060 8GB 포함)의 ASM2464PD USB4/PCIe 브리지 C 펌웨어를 오픈소스로 공개. USB4로 PCIe Gen4 x4 지원; USB3 DMA ~700 MB/s. openpilot 0.11.2(1B 주행 모델, 기존 대비 파라미터 30×)와 동시 출시. AI 관련 하드웨어 최초 오픈소스 컨트롤러 펌웨어; clawfit 하드웨어 축에 `laptop_egpu_usb4` 토폴로지 갭 확인. | L7/L1 오픈 펌웨어 eGPU |
| **[jundot/oMLX ⭐18,791](https://github.com/jundot/omlx) (GitHub Trending Python, 2026-08-16)** | Apple Silicon 추론 서버: 계층형 KV 캐시(RAM 핫→SSD 콜드, 재시작 후에도 KV 블록 유지), 다중 모델 LRU 서빙, OpenAI + Anthropic API를 단일 프로세스로 제공. 메뉴 바 + 관리자 대시보드. 추적된 Apple Silicon 추론 서버 중 RAM→SSD KV 퇴출 경로를 문서화한 첫 사례 — `hardware: local_mac` 프로필의 유효 컨텍스트 용량을 "RAM 한계" 에서 "SSD 한계"로 확장. | L1/L7 Apple Silicon 멀티 모델 추론 |
| **[google-research/timesfm ⭐27,646](https://github.com/google-research/timesfm) + AGENTS.md (GitHub Trending, 2026-08-16)** | 제로샷 시계열 예측 전용 200M 파라미터 디코더 전용 트랜스포머(TimesFM 2.5). 2026년 3월 AGENTS.md 추가 — 에이전트가 모델을 호출하는 방법을 기계 판독 가능 스펙으로 문서화. BigQuery ML·Vertex·구글 스프레드시트에서 사용 가능. LLM이 아닌 도메인 모델이 AGENTS.md를 추가해 에이전트 호출 가능 L4 능력이 되는 패턴 최초 확인. | L1/L4 도메인 모델 + AGENTS.md |
| **[THUDM/slime ⭐8,059](https://github.com/THUDM/slime) (GitHub Trending Python, 2026-08-16)** | Megatron(학습)과 SGLang(롤아웃)을 Data Buffer로 연결하는 RL 사후 학습 프레임워크 — 추상화 심 없이 양쪽 엔진에 네이티브 인수 전달. 멀티 에이전트 RL·도구 사용 학습·장기 태스크 RL 지원. 프리필/디코드 분리 및 델타 웨이트 동기화 제공. GLM-4.7~5.2 프로덕션 학습 인프라. 독립 문서 최초 작성; 이전에는 OpenClaw-RL 업스트림 의존성으로만 언급. | L1 RL 사후 학습 프레임워크 |
| **[unslothai/Unsloth Desktop ⭐71,400+](https://unsloth.ai/docs/desktop) (GeekNews, 2026-08-14)** | 무료 Tauri 데스크탑 앱: 로컬 모델 실행 + 노코드 LoRA 파인튜닝 + 원 커맨드 에이전트 브리지(`unsloth start claude`로 로컬 GGUF/MLX 모델을 Claude Code에 연결). Cloudflare HTTPS 터널로 원격 서빙 지원. 파인튜닝→로컬 실행→에이전트 브리지를 단일 데스크탑 워크플로로 통합한 최초 도구. | L7/L2 로컬 모델 허브 + 에이전트 브리지 |
| **[Docker Sandboxes](https://www.docker.com/blog/introducing-docker-sandboxes-for-ai-coding-agents/) (HN 518점, 2026-08-10)** | Docker 최초 네이티브 에이전트 실행 안전 제품: 마이크로VM 격리로 `--dangerously-skip-permissions` 자율성을 제한된 폐기 가능 환경 안에서 허용. Claude Code·Gemini CLI·Copilot CLI·Codex·OpenCode·Kiro 지원. AI Governance 확장으로 조직 전체 MCP 정책 강제. Docker + Anthropic sandbox-runtime + NVIDIA NemoClaw 3개 벤더 에이전트 격리 클러스터 확인. | L7/L2 에이전트 실행 격리 |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (603개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
