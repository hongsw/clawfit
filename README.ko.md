...
## 🔥 지금 가장 뜨거운 것들 (2026-08-23)

| 신호 | 왜 중요한가 | 레벨 |
|------|------------|------|
| **[Untrivial-ai/agent-orchestrator ⭐9,900](https://github.com/Untrivial-ai/agent-orchestrator) (GitHub Trending, 2026-08-23)** | Claude Code·GitHub Copilot·Cursor·Aider 등 26+ 코딩 에이전트를 동시에 조율하는 데스크탑 플릿 관리 IDE. 태스크별 격리 git 워크트리, 자율 CI 수정·머지 충돌 해소·PR 코드 리뷰, 라이브 칸반 보드. 에이전트 위에서 에이전트를 관리하는 `fleet_controller` 차원 갭 확인. Apache 2.0. | L2 플릿 코디네이터 / L3 팀 워크플로 |
| **hermes-agent v0.20.0 ⭐3,200 (GitHub, 2026-08-23)** | TTFT ~80% 감소 + 실시간 추론 스트림 기본 활성화(Quicksilver 업데이트). 여섯 번째 hermes 시그널; latency 재분류 대기 중. 자기 개선 루프를 갖춘 L1 기반 런타임의 대표 사례. | L1 기반 런타임 |
| **Agent Plugins v1.0 스펙 신호 (2026-08-23)** | 크로스 에이전트 플러그인 마켓플레이스 사양 첫 안정 릴리즈. 하네스 독립 플러그인 생태계 형성 신호 — wshobson/agents와 함께 두 번째 교차일 확인. | L4b 크로스 에이전트 스킬팩 |
| **[KunAgent/Kun ⭐6,200](https://github.com/KunAgent/Kun) (GitHub Trending, 2026-08-23)** | 로컬 퍼스트 AI 에이전트 워크스페이스: 데스크탑 Electron GUI와 TUI가 단일 런타임을 공유 — 스레드·목표·승인 큐 실시간 동기화. MCP 네이티브, 멀티 LLM(로컬 포함) 지원. Code/Work 이중 모드. `cli+gui` 하이브리드 인터페이스 서브타입 첫 시그널. | L1 기반 런타임 / L2 워크스페이스 |
| **TencentDB-Agent-Memory v2.0 ⭐8,200 (GitHub, 2026-08-23)** | 4계층 프로그레시브 에이전트 메모리 2.0 업데이트. 장기·단기·에피소드·의미 기억 레이어 분리. mem0(53.5k)·hindsight(19.4k)와 함께 "구조화된 에이전트 메모리 인프라" 세 번째 확인. | L4a 에이전트 메모리 |
| **MCP Roadmap Aug 2026 (공식 블로그, 2026-08-23)** | MCP 2026년 하반기 로드맵 공개: Stateless RC 후속 정식 사양, 멀티 서버 컴포지션, 서버 검색 표준화. L4c 생태계 전반에 걸친 구조 변화 예고. | L4c MCP 생태계 |
| **[OzBrain ⭐4,100](https://github.com/australia-ai/ozbrain) (GitHub Trending, 2026-08-23)** | 호주 정부 후원 오픈 에이전트 런타임: 공공 서비스·법률·행정 도메인 특화, Apache 2.0. 코퍼스 내 정부 주도 오픈 에이전트 런타임 첫 사례. DOE Genesis(미국)에 이어 두 번째 정부 AI 이니셔티브 시그널. | L1/L2 정부 이니셔티브 에이전트 |
| **Stripe × OpenRouter M&A 시그널 (2026-08-23)** | Stripe가 OpenRouter(231+ LLM 프로바이더 게이트웨이)를 인수 협상 중이라는 업계 보도. 성사 시 결제 인프라와 LLM 라우팅이 단일 스택으로 통합 — `api_routing` + `payment_integration` 전략 신호. | L7 인프라 전략 |

전체 분석: [`docs/research-watch/`](docs/research-watch/) (635개 문서) · 전체 맵: [`docs/reference-levels.md`](docs/reference-levels.md)
